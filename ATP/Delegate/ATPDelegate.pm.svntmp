...
sub getData {
    ...
    my $customerNumberMap
        = CNDB::Delegate::CNDBDelegate->getCustomerNumberMap;
    ...
    if ( $delta == 1 ) {
        $connection->prepareSqlQueryAndFields(
            $self->queryATPDeltaData($testMode) );
    }
    else {
        $connection->prepareSqlQueryAndFields(
            $self->queryATPData($testMode) );
    }
    ...
    $sth->bind_columns( map { \$rec{$_} }
            @{ $connection->sql->{atpDataFields} } );
    dlog('Columns binded');

    dlog('Executing query');
    if ( $delta == 1 ) {
        dlog(     'Executing delta query '
                . "HWDATE: $hwDate LPARDATE: $lparDate" );
    ...
    while ( $sth->fetchrow_arrayref ) {
        ...
        if ( $customerNumberMap->{ $rec{customerNumber} }->{'count'} == 1 ) {
            ...
            foreach my $countryCode (
                keys %{ $customerNumberMap->{ $rec{customerNumber} } } )
            {
                next if $countryCode eq 'count';
                $hwCustomerId = $customerNumberMap->{ $rec{customerNumber} }
                    ->{$countryCode};
            }
        }
        elsif (
            defined $customerNumberMap->{ $rec{customerNumber} }
            ->{ $rec{country} } )
        {
        ...
        if ( !defined $customerNumberMap->{ $rec{lparCustomerNumber} } ) {
            dlog(
                'LPAR customer number is not defined in cndb, reverting to hardware customer number'
            );
            $hwLparCustomerId = $hwCustomerId;
        }
        elsif (
            $customerNumberMap->{ $rec{lparCustomerNumber} }->{'count'} == 1 )
        {
            dlog('Lpar Customer number is unique, assigning to hardware');
            foreach my $countryCode (
                keys %{ $customerNumberMap->{ $rec{lparCustomerNumber} } } )
            {
                next if $countryCode eq 'count';
                $hwLparCustomerId
                    = $customerNumberMap->{ $rec{lparCustomerNumber} }
                    ->{$countryCode};
            }
        }
        elsif (
            defined $customerNumberMap->{ $rec{lparCustomerNumber} }
            ->{ $rec{country} } )
        {
            dlog("Matching country code found; assigning customer id");
            $hwLparCustomerId
            ...
            dlog(
                "No unique customer number/country code defined for lpar, assigining hw customer id"
            );
            $hwLparCustomerId = $hwCustomerId;
        }
        ...
        ###Construct the key
        my $key
            = $machineTypeMap->{ $rec{machineType} } . '|'
            . $rec{serial} . '|'
            . $rec{country};
        ...
        if ( !exists $hardwareList{$key} ) {
            ...
            $hardware->serverType(
                $self->getServerType( $rec{serverType}, undef ) );
            $hardwareList{$key} = $hardware;
        }
        else {
            $hardwareList{$key}->serverType(
                $self->getServerType(
                    $rec{serverType}, $hardwareList{$key}->serverType
                )
            );
