...
my $logfile         = "/var/staging/logs/adcToBravo/adcToBravo.log";
my $pidFile         = "/tmp/adcToBravo.pid";
my $configFile      = "/opt/staging/v2/config/adcToBravoConfig.txt";
...
eval {
    ...
    while (1) {
        ...
        $loadDeltaOnly = 1;
        if ( SystemScheduleStatusDelegate->isFullLoad( "$job (FULL)",
                                                   $fullLoadPeriodHours ) == 0
            )
        {
        ...
        if ( $applyChanges == 1 ) {
            ...
            $systemScheduleStatus
                = SystemScheduleStatusDelegate->start($job1);
