...
public abstract class BaseAlertUnlicensedSwServiceImpl extends
    ...
    public final String SQL_SELECT_UNLICENSED_SW_BY_ACCOUNT_ID_AND_TYPE = "SELECT Alert_Age, Software_Item_Name, Alert_Count, Creation_Time FROM (SELECT MAX(DAYS(CURRENT TIMESTAMP) - DAYS(VA.Creation_Time)) AS Alert_Age, SI.Name AS Software_Item_Name, COUNT(*) AS Alert_Count, MIN(VA.Creation_Time) AS Creation_Time FROM EAADMIN.V_Alerts VA, EAADMIN.Product P, EAADMIN.Alert_Unlicensed_Sw AUS, EAADMIN.Installed_Software IS, EAADMIN.Software_Item SI WHERE VA.Customer_Id = :accountId AND VA.Type = :type AND VA.Open = 1 AND AUS.Id = VA.Id AND IS.Id = AUS.Installed_Software_Id AND IS.Software_Id = P.Id AND SI.Id = P.Id GROUP BY Name) AS TEMP";

    public final String SQL_SELECT_UNLICENSED_SW_BY_REMOTE_USER_AND_TYPE = "SELECT Alert_Age, Software_Item_Name, Alert_Count, Creation_Time FROM (SELECT MAX(DAYS(CURRENT TIMESTAMP) - DAYS(VA.Creation_Time)) AS Alert_Age, SI.Name AS Software_Item_Name, COUNT(*) AS Alert_Count, MIN(VA.Creation_Time) AS Creation_Time FROM EAADMIN.V_Alerts VA, EAADMIN.Product P, EAADMIN.Alert_Unlicensed_Sw AUS, EAADMIN.Installed_Software IS, EAADMIN.Software_Item SI WHERE VA.Remote_User = :remoteUser AND VA.Type = :type AND VA.Open = 1 AND AUS.Id = VA.Id AND IS.Id = AUS.Installed_Software_Id AND IS.Software_Id = P.Id AND SI.Id = P.Id GROUP BY Name) AS TEMP";
