...
public class ReportServiceImpl implements ReportService {
	...
	private final String[] FULL_RECONCILIATION_REPORT_COLUMN_HEADERS = {
			...
			"Hardware IFL", "Shared processor", "CPU IBM LSPR MIPS",
			"CPU Gartner MIPS", "CPU MSU", "Part IBM LSPR MIPS",
	...
	private final String[] LICENSE_COLUMN_HEADERS = { "Primary Component",
			...
			"SWCM ID", "SKU", "Pool", "Record date/time" };
	...
	public void getFullReconciliationReport(Account pAccount,
		...
		String lsBaseFromClause = "from  "
				...
				+ "left outer join eaadmin.installed_tadz insTadz on "
				+ "is.id = insTadz.installed_software_id "
				+ "left outer join eaadmin.bank_account ba on "
				+ "insTadz.bank_account_id =  ba.id "
	...
	public void getLicenseBaselineReport(Account pAccount, String remoteUser,
			...
			boolean pbTitlesNotSpecifiedInContractScopeSearchChecked,
			boolean pbSelectAllChecked) throws HibernateException, Exception {
		String lsBaseSelectAndFromClause = "SELECT COALESCE(S.software_name, L.Full_Desc) AS Product_Name, CASE LENGTH(COALESCE(S.software_name, '')) WHEN 0 THEN 'No' ELSE 'Yes' END, L.Full_Desc, L.PID as swproPID, CONCAT(CONCAT(RTRIM(CHAR(CT.Code)), '-'), CT.Description), L.environment, L.Quantity, coalesce(L.Quantity - sum(VLUQ.Used_Quantity), L.Quantity), L.Expire_Date,L.END_DATE, L.Po_Number, L.Cpu_Serial, CASE L.IBM_Owned WHEN 1 THEN 'IBM' ELSE 'Customer' END, L.Ext_Src_Id, L.SKU, CASE L.Pool WHEN 0 THEN 'No' ELSE 'Yes' END, L.Record_Time FROM EAADMIN.License L LEFT OUTER JOIN EAADMIN.License_Sw_Map LSWM ON LSWM.License_Id = L.Id LEFT OUTER JOIN EAADMIN.software S ON S.software_id = LSWM.Software_Id LEFT OUTER JOIN EAADMIN.USED_LICENSE VLUQ ON VLUQ.License_Id = L.Id, EAADMIN.Capacity_Type CT";
		...
		lsbSql.append("group by COALESCE(S.software_name, L.Full_Desc), CASE LENGTH(COALESCE(S.software_name, '')) WHEN 0 THEN 'No' ELSE 'Yes' END, L.Full_Desc, L.PID, CONCAT(CONCAT(RTRIM(CHAR(CT.Code)), '-'), CT.Description),L.environment, L.Quantity, L.Expire_Date,L.END_DATE, L.Po_Number, L.Cpu_Serial, CASE L.IBM_Owned WHEN 1 THEN 'IBM' ELSE 'Customer' END, L.Ext_Src_Id, L.SKU, CASE L.Pool WHEN 0 THEN 'No' ELSE 'Yes' END, L.Record_Time ");
		...
		printHeader(LICENSE_BASELINE_REPORT_NAME, pAccount.getAccount(),
				LICENSE_BASELINE_COLUMN_HEADERS, pPrintWriter);
		while (lsrReport.next()) {
			pPrintWriter.println(outputData(lsrReport.get()));
		}
		lsrReport.close();
