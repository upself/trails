...
public class DataExceptionTypeDaoJpa extends
	...
	public List<Map<String, String>> summary(final Long accountId,
		...
		if (alertTypeType.equals("DATA_EXCEPTION")) {
			...
			result = entityManager
					.createQuery(
							"select new map('ALERT' as type, a.type as code, a.displayName as name, count(a.id) as total) from AlertView a where a.displayName not like ('%SOM%') and a.account.id = :account and a.open = 1 group by 1, a.type, a.displayName order by count(a.id) desc")
					.setParameter("account", accountId)
					.setHint("org.hibernate.cacheable", Boolean.TRUE)
					.getResultList();
