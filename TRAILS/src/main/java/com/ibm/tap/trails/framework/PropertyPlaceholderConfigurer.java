 ...
 * explicit placeholders in context definitions. Therefore, the original
 * definition cannot specify any default values for such bean properties, and
 * the placeholder properties file is supposed to contain an entry for each
 * defined placeholder.
 * 
 * <p>
 * If a configurer cannot resolve a placeholder, a BeanDefinitionStoreException
 * will be thrown. If you want to check against multiple properties files,
 * specify multiple resources via the "locations" setting. You can also define
 * multiple PropertyPlaceholderConfigurers, each with its <i>own</i> placeholder
 * syntax.
 * 
 * <p>
 * Default property values can be defined via "properties", to make overriding
 * definitions in properties files optional. A configurer will also check
 * against system properties (e.g. "user.dir") if it cannot resolve a
 * placeholder with any of the specified properties. This can be customized via
...
public class PropertyPlaceholderConfigurer extends PropertyResourceConfigurer
	 ...
	 * associated default value, or <code>null</code> if no such special
	 * character should be processed as a value separator. The default is ":".
	 */
	...
	protected String parseStringValue(String strVal, Properties props,
		...
		PlaceholderResolver resolver = new PropertyPlaceholderConfigurerResolver(
				props);
