...
<h1>SW LPAR w/o HW LPAR alerts: <s:property value="account.name" />(<s:property
	value="account.account" />)</h1>
<p class="confidential">IBM Confidential</p>
<br />
<p>This page displays software lpars without an associated hardware
lpar. Use the checkboxes to assign, update or unassign alerts. You must
enter a comment to successfully update the alert.</p>
<br />
<div class="download-link" style="float:right">
	<s:url id="reportUrl"
		value="/TRAILS/report/download/alertSoftwareLpar%{#attr.account.account}.tsv?name=alertSoftwareLpar"
		includeContext="false" includeParams="none" />
	<s:a href="%{reportUrl}">Download SW LPAR w/o HW LPAR alert report</s:a>
</div>
<br />
<br />
...
<s:form action="alertSoftwareLpar" method="post"
	...
	<display:table name="data" class="basic-table" id="row"
	 summary="SoftdwareLpar Without HardwareLpar Alerts "
		decorator="org.displaytag.decorator.TotalTableDecorator"
		cellspacing="1" cellpadding="0" requestURI="alertSoftwareLpar.htm">
		...
		<display:column sortProperty="softwareLpar.name" title="Name"
			...
			<s:url id="bravoUrl"
				...
				includeContext="false" includeParams="none"  />
			<s:a id="software_lpar_name_%{#attr.row.softwareLpar.id}" href="%{bravoUrl}">${row.softwareLpar.name}</s:a>
		</display:column>
		...
		<display:column title="Comments">
			<s:url id="commentsUrl"
				value="javascript:displayPopUp('alertSoftwareLparHistory.htm?id=%{#attr.row.tableId}')"
				includeContext="false" includeParams="none" />
