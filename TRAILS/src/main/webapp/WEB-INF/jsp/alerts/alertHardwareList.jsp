...
<div class="ibm-columns">
	<div class="ibm-col-1-1">
		...
		<p>This page displays hardware without an associated hardware lpar. Use the checkboxes to assign,update or unassign alerts. You must enter a comment to successfully update the alert.</p>
		<div style="text-align:right">
			<a href="${pageContext.request.contextPath}/ws/alertHardware/download/${accountId}">Download SOM1a: HW WITH HOSTNAME alert report</a>
		</div>
	</div>
	...
	<!-- SORTABLE DATA TABLE -->
	<div class="ibm-col-1-1">
		<table id="page" cellspacing="0" cellpadding="0" border="0" class="ibm-data-table ibm-alternating" summary="SOM1a: Hardware with Hostname">
			<thead>
				<tr>
					...
					<th scope="col" class="ibm-sort nobreak"><a href="javascript:void(0)"><span>MachineType</span><span class="ibm-icon"></span></a></th>
					<th scope="col" class="ibm-sort nobreak"><a href="javascript:void(0)"><span>Country</span><span class="ibm-icon"></span></a></th>
...
function searchData(){
	...
	$("#page").paginationTable('destroy').paginationTable({
		remote: {
			url: "${pageContext.request.contextPath}/ws/alertHardware/search",
			type: "POST",
			params: params,
			success: function(result, pageIndex){
				...
				if(null == list || list == undefined || list.length == 0){
					...
					for(var i = 0; i < list.length; i++){
						...
						html += "<td><a href='javascript:void()' onclick='popupHardware(" + list[i].account.account + ");return false;'>" + list[i].hardware.serial + "</a></td>";
						html += "<td>" +list[i].hardware.machineType.name + "</td>";
						html += "<td>" +list[i].hardware.country + "</td>";
						html += "<td>" + list[i].creationTime + "</td>";
						html += "<td>" + list[i].alertAge + "</td>";
						html += "<td>" + list[i].remoteUser + "</td>";
						html += "<td><a href='javascript:void()' onclick='displayPopUp(\"alertHardwareHistory.htm?id="+list[i].tableId+"\");return false;'>View</a></td>";
						html += "</tr>";
					}
				}
				$("#tb").html(html);
		...
		orderColumns: ['tableId','alertAge','hardware.serial','hardware.machineType.name','hardware.country','creationTime','alertAge','remoteUser','tableId']
	}); 
};
function assignComments(type){
	
	var comments = $('#comments').val();
	var url = '${pageContext.request.contextPath}/ws/alertHardware/';
	var params = {};
