					...
					for (var i = 0; i < list.length; i++) {
						if (i == 0){
							$('#alertTypename')
					         ...
					         .attr({"value":list[i].name,"id":list[i].id,"selected":"selected"})
					         .text(list[i].name)); 	
							$("#alertTypeId").val(list[i].id) ;
						} else {
						  $('#alertTypename')
					         .append($("<option></option>")
					         .attr({"value":list[i].name,"id":list[i].id})
					         .text(list[i].name)); 
						}
					}
				},
...
function callRestApi() {
	...
	var obj =  "{\"pk\":{\"alertType\":{\"id\":"+ $("#alertTypeId").val() +",\"name\":\" "+ $("#alertTypename").val() +"\",\"code\":\"\"},\"alertCause\":{\"id\":"+ $("#causeTypeId").val() +",\"name\":\" "+ $("#causeType").val() +"\",\"showInGui\":true,\"alertCauseResponsibility\":{\"id\":"+ $("#responsibilityId").val() +" ,\"name\":\" "+ $("#responsibility").val() +"\"}}},\"status\":\""+ $("#status").val() +"\"}";
...
<br />
<p>Add the cause code's name. Press the Save button to save your changes. Fields marked with an asterisk (*) are required.</p>
<br />
		...
		<span class="ibm-spinner-large" id="loading" style="display:none"></span>
			...
			<p>
				<label for="alertTypename">Alert:<span class="ibm-required">*</span>
						...
						<select name="alertTypename" id="alertTypename" style="max-width:270px;">
					</select> 
					</span>
