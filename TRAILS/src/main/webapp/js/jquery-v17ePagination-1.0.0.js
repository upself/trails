...
!function($) {
	...
	var Page = function(element, options) {
		...
		this.options = $.extend(true, {}, defaultOption,
				$.fn.v17ePagination.defaults, options);
		...
		var init = function(obj) {
			...
			// init size
			that.$sizeInfo.append('Results per page:<strong>'
					+ that.currentPageSize + '</strong>');
			for (var i = 0; i < that.options.pageSizeItems.length; i++) {
				if (that.options.pageSizeItems[i] != that.currentPageSize) {
					that.$sizeLinks
							.append('<a style="cursor:pointer; margin-left:3px;" class="ibm-forward-em-link" data-page-size="'
									+ that.options.pageSizeItems[i]
									+ '">'
			...
			// init CSS
			that.$element.parent().css('position', 'relative');
		...
		} else if (typeof this.options.total === 'undefined'
				&& !this.options.loadFirstPage) {
			console
					&& console
							.error("[init error] : if you don't remote the first page. you must set the options or 'total'.");
		} else {
			init(this);
		}
	}
	...
	Page.prototype = {
		_remoteOrRedner : function(pageIndex) {
			if (this.options.remote.url != null
					&& (this.options.loadFirstPage || pageIndex > 1)) {
			...
			$
					.ajax({
						...
						success : function(result) {
							that.debug("ajax request : params = "
									+ JSON.stringify(requestParams), result);
		...
		renderPagination : function(pageIndex) {
			...
			this.$info.text(renderInfo(this.currentPageIndex,
					this.currentPageSize, this.total, this.options.infoFormat));
			if (this.pageCount >= 1) {
				this.$page.show();
		...
		showLoading : function() {
			/*
			 * var height = this.$container.height(); var width =
			 * this.$container.width(); var top =
			 * this.$container.position().top; var left =
			 * this.$container.position().left; console.info("width=" +width +",
			 * height=" + height + ", top=" + top + ", left="+left);
			 */
			this.$loading.css("cssText", "height: "
					+ (this.$container.height() || 50) + "px !important");
			this.$loading.css({
				"position" : "absolute",
	...
	var renderPages = function(pageIndex, pageSize, total, pageBtnCount,
			...
			showFirstLastBtn) {
		pageIndex = pageIndex == undefined ? 1 : parseInt(pageIndex); // set
																		// pageIndex
																		// from
																		// 1，
																		// convenient
	...
	var renderPerPage = function(text, value) {
		return $("<a style='cursor:pointer; margin-left:3px;' class='ibm-forward-em-link' data-page-index='"
				+ value + "'>" + text + "</a>");
	}
