package BRAVO::CustomerTypeLoader;
...
sub load {
    ...
    my $job = "CUSTOMER TYPE REPLICATION";
    dlog("job=$job");
...
sub prepareSourceData {
    ...
    ilog('Acquring customer type data');

    $self->list( CNDB::Delegate::CNDBDelegate->getCustomerTypeData($connection) );
...
sub doDelta {
    ...
    dlog('Preparing bravo customer type query');
    $connection->prepareSqlQueryAndFields(    
                                           BRAVO::Delegate::BRAVODelegate->queryCustomerTypeData()
    );
    dlog('bravo customer type query prepared');
    ...
    my %rec;
    $sth->bind_columns( map { \$rec{$_} } @{ $connection->sql->{customerTypeDataFields} } );
    dlog('Columns binded');

    ilog('Executing bravo customer type query');
    $sth->execute();
    while ( $sth->fetchrow_arrayref ) {

        ###Get the key
        my $key = $rec{id};
        dlog("customer type key=$key");

        ###Create and populate a new CustomerType object
        my $customerType = new BRAVO::OM::CustomerType();
        $customerType->id( $rec{id} );
        ...
        dlog( $customerType->toString );

        if ( defined $self->list->{$key} ) {
            dlog('customer type exists in cndb');

            $self->list->{$key}->id( $rec{id} );

            if ( !$customerType->equals( $self->list->{$key} ) ) {
                dlog('customer type is not equal');
                dlog( $self->list->{$key}->toString );

                $self->list->{$key}->action('UPDATE');
            }
            else {
                dlog('customer type is equal, setting to complete');
                $self->list->{$key}->action('COMPLETE');
            }
        }
        else {
            elog('customer type does not exist in cndb');

            #			die('customer type deleted from CNDB');
        }
    }
    $sth->finish();
