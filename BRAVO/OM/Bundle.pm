...
sub save {
    ...
    if( ! defined $self->id ) {
        ...
        my $sth = $connection->sql->{insertBundle};
        my $id;
        $sth->bind_columns(\$id);
        $sth->execute(
            $self->softwareId
        ...
        my $sth = $connection->sql->{updateBundle};
        $sth->execute(
            $self->softwareId
            ,$self->id
        );
...
sub delete {
    ...
    if( defined $self->id ) {
        ...
        my $sth = $connection->sql->{deleteBundle};
        $sth->execute(
            $self->id
        );
        $sth->finish;
...
sub queryDelete {
    my $query = '
        delete from bundle a where
            a.id = ?
    ';
    return ('deleteBundle', $query);
}

sub getById {
    my($self, $connection) = @_;
    $connection->prepareSqlQuery($self->queryGetById());
    my $sth = $connection->sql->{getByIdKeyBundle};
    my $softwareId;
    $sth->bind_columns(
        \$softwareId
    );
