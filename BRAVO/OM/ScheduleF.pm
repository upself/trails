...
sub save {
    ...
    if( ! defined $self->id ) {
        ...
        my $sth = $connection->sql->{insertScheduleF};
        my $id;
        $sth->bind_columns(\$id);
        $sth->execute(
            $self->softwareId
        ...
        my $sth = $connection->sql->{updateScheduleF};
        $sth->execute(
            $self->softwareId
            ,$self->id
        );
...
sub queryInsert {
    my $query = '
        ...
        from
            final table (
        insert into schedule_f (
            software_id
        ) values (
            ?
        ))
    ';
    return ('insertScheduleF', $query);
}
...
sub delete {
    ...
    if( defined $self->id ) {
        ...
        my $sth = $connection->sql->{deleteScheduleF};
        $sth->execute(
            $self->id
        );
        $sth->finish;
...
sub queryDelete {
    my $query = '
        delete from schedule_f a where
            a.id = ?
    ';
    return ('deleteScheduleF', $query);
}

sub getById {
    my($self, $connection) = @_;
    $connection->prepareSqlQuery($self->queryGetById());
    my $sth = $connection->sql->{getByIdKeyScheduleF};
    my $softwareId;
    $sth->bind_columns(
        \$softwareId
    );
