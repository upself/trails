 ...
 * 
 */
public class SoftAuditImporter implements InterfaceParser {
	...
	.getLogger(SoftAuditImporter.class);
    ...
    public boolean parse(SwScan mySwScan ) throws IOException {
        ...
        while (( (str = csvr.readNext()) != null) && (str.length > 9)) {
        	if ( (str.length < 11) ||
        			((str[0] == null) || (str[0].length() < 1) ) ||
					((str[6] == null)) || (str[6].length() < 1 )  ) {
        		errorMessage = "Invalid file format in SoftAudit Parsing file: " + mySwScan.getFileName() + " at "
				+ line_count;
