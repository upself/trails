...
public abstract class DelegateHardware extends HibernateDelegate {
	...
	public static List<HardwareLpar> search(String search, Account account,
		...
		list = session.getNamedQuery("hardwareLparsSearch")
				.setEntity("customer", account.getCustomer())
				.setString("name", "%" + search.toUpperCase() + "%")
				.setString("serial", "%" + search.toUpperCase() + "%")
	...
	public static List<HardwareLpar> search(String search) throws Exception {
		...
		list = session.getNamedQuery("hardwareLparsSearchAll")
				.setString("name", "%" + search.toUpperCase() + "%")
				.setString("serial", "%" + search.toUpperCase() + "%").list();
	...
	public static List<Hardware> searchHardwareNoLpars(String search,
		...
		list = session.getNamedQuery("hardwareNoLparsSearch")
				.setEntity("customer", account.getCustomer())
				.setString("serial", "%" + search.toUpperCase() + "%")
				.setParameterList("status", status).list();
	...
	public static List<Hardware> searchHardwareNoLpars(String search,
		...
		list = session
				...
				.setParameterList("hwStatus", hwStatus).list();
	...
	public static Integer searchHardwareNoLparsSize(String search)
		...
		count = (Integer) session.createCriteria(Hardware.class)
				.setProjection(Projections.rowCount())
				.setFetchMode("machineType", FetchMode.JOIN)
				.add(Restrictions.isEmpty("hardwareLpars"))
				.add(Restrictions.like("serial", search, MatchMode.ANYWHERE))
				.uniqueResult();
	...
	@SuppressWarnings("unchecked")
	public static List<Hardware> searchHardwareNameNoLpars(String search)
			throws Exception {
		logger.debug("DelegateHardware.search search = " + search);
		List<Hardware> list = null;
		...
		list = session.createCriteria(Hardware.class)
				.setFetchMode("machineType", FetchMode.JOIN)
				.add(Restrictions.isEmpty("hardwareLpars"))
				.add(Restrictions.eq("serial", search)).list();
	...
	@SuppressWarnings("unchecked")
	public static List<Hardware> searchHardwareSerialNoLpars(String search)
			throws Exception {
		logger.debug("DelegateHardware.search search = " + search);
		List<Hardware> list = null;
	...
	public static Integer searchLparNoCompositeNameSize(String search)
			throws HibernateException, Exception {
		Integer count = null;
		...
		count = (Integer) session.createCriteria(HardwareLpar.class)
				...
				.add(Restrictions.isNull("softwareLpar"))
	...
	public static Integer searchLparSerialNoCompositeSize(String search)
		...
		count = (Integer) session.createCriteria(HardwareLpar.class)
				...
				.setFetchMode("customer", FetchMode.JOIN)
				.add(Restrictions.isNull("softwareLpar"))
				.add(Restrictions.eq("h.serial", search)).uniqueResult();
	...
	@SuppressWarnings("unchecked")
	public static List<HardwareLpar> searchLparNameNoComposite(String search)
			throws Exception {
		logger.debug("DelegateHardware.search search = " + search);
		List<HardwareLpar> list = null;
	...
	@SuppressWarnings("unchecked")
	public static List<HardwareLpar> searchLparSerialNoComposite(String search)
			throws Exception {
		logger.debug("DelegateHardware.search search = " + search);
		List<HardwareLpar> list = null;
	...
	public static List<HardwareLpar> searchLparNoComposite(String search,
		...
		if (search.length() >= 3) {
			list = session
					...
					.add(Restrictions.in("h.hardwareStatus", hwStatus))
	...
	public static Hardware getHardware(long hardwareId) throws Exception {
		...
		Hardware hw = (Hardware) session.getNamedQuery("hardwareByID")
				.setLong("hardwareId", hardwareId).uniqueResult();
	...
	public static Hardware getHardware(Account account, String mt, String sn5)
		...
		Hardware hw = (Hardware) session
				.getNamedQuery("hardwareByAccountByMtBySn5")
				.setEntity("customer", account.getCustomer())
	...
	public static List<HardwareLpar> getHardwareLpars(Account account) {
		...
		try {
			...
			list = session.getNamedQuery("hardwareLparsByAccount")
					.setEntity("customer", account.getCustomer()).list();
	...
	public static HardwareLpar getHardwareLpar(String lparName, Account account) {
		...
		try {
			...
			hardwareLpar = (HardwareLpar) session
					.getNamedQuery("hardwareLparByAccountByName")
					.setEntity("customer", account.getCustomer())
	...
	public static SCRTRecord saveSCRTRecord(SCRTRecord scrtRecord,
		...
		try {
			...
			dbScrtRecord = (SCRTRecord) session
					.getNamedQuery("scrtRecordByHardwareByYearByMonthByLpar")
					.setEntity("hw", scrtRecord.getHardware())
					.setInteger("year", scrtRecord.getYear().intValue())
					.setInteger("month", scrtRecord.getMonth().intValue())
					.setString("lpar", scrtRecord.getLpar()).uniqueResult();
	...
	public static boolean deleteSCRTRecord(Hardware hw, String reportYear,
		...
		try {
			...
			dbScrtRecord = (SCRTRecord) session
					.getNamedQuery("scrtRecordByHardwareByYearByMonthByLpar")
					.setEntity("hw", hw)
					.setInteger("year", new Integer(reportYear).intValue())
	...
	public static List<SCRTRecord> getSCRTRecords(Hardware hw) throws Exception {
		...
		list = session.getNamedQuery("scrtRecordsByHardware")
				.setEntity("hw", hw).list();
	...
	public static Collection<HardwareLpar> getHardwareLparsWithoutSoftware(
		...
		list = session.getNamedQuery("hardwareLparsWithoutSoftware")
				.setEntity("customer", account.getCustomer()).list();
	...
	public static Long getLparNoCompositeByCustomerByStatusSize(
		...
		count = (Long) session
				.getNamedQuery("getLparNoCompositeByCustomerByStatusSize")
				.setEntity("customer", account.getCustomer())
				.setParameterList("status", status)
	...
	public static Long getHardwaresNoLparsByCustomerByStatusSize(
		...
		count = (Long) session
				.getNamedQuery("getHardwaresNoLparsByCustomerByStatusSize")
				.setEntity("customer", account.getCustomer())
				.setParameterList("hardwareStatus", hwStatus).uniqueResult();
	...
	@SuppressWarnings("unchecked")
	public static List<Hardware> getHardwaresNoLparsByCustomerByStatus(
			Customer customer, List<String> hwStatus)
			throws HibernateException, Exception {
		List<Hardware> list = null;
	...
	public static List<Hardware> getHardwaresNoLparsByCustomerByStatusSerial(
		...
		if (serial.length() >= 3) {
			list = session
					.getNamedQuery("hardwaresNoLparsByCustomerByStatusSerial")
					.setEntity("customer", customer)
					.setString("serial", "%" + serial + "%")
	...
	public static List<HardwareLpar> searchHardwareLparsWithAuthorizedProductsByCustomer(
		...
		if (search.length() >= 3) {
			list = session
					...
					.createAlias("hardware", "h")
					.add(Restrictions.eq("customer", account.getCustomer()))
					.add(Restrictions.isNotEmpty("authorizedProducts"))
					.add(Restrictions
							.disjunction()
							.add(Restrictions.like("name", search,
									MatchMode.ANYWHERE).ignoreCase())
