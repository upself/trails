package BRAVO::CustomerNumberLoader;
...
sub load {
    ...
    my $job = "CUSTOMER NUMBER REPLICATION";
    dlog("job=$job");
...
sub doDelta {
    ...
    while ( $sth->fetchrow_arrayref ) {
        ...
        ###Get the key
        my $key = $rec{id};
        dlog("customer number key=$key");

        ###Create and populate a new customer number object
        my $customerNumber = new BRAVO::OM::CustomerNumber();
        $customerNumber->id( $rec{id} );
        ...
        dlog( $customerNumber->toString );

        if ( defined $self->list->{$key} ) {
            dlog('customer number exists in cndb');

            $self->list->{$key}->id( $rec{id} );

            if ( !$customerNumber->equals( $self->list->{$key} ) ) {
                dlog('customer number is not equal');
                dlog( $self->list->{$key}->toString );

                $self->list->{$key}->action('UPDATE');
            }
            else {
                dlog('customer number is equal, setting to complete');
