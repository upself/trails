...
sub prepareSourceData {
    ...
    ilog('Acquring cndb region data');

    $self->list( CNDB::Delegate::CNDBDelegate->getRegionData($connection) );
...
sub doDelta {
    ...
    dlog('Preparing bravo region query');
    $connection->prepareSqlQueryAndFields(    
                                           BRAVO::Delegate::BRAVODelegate->queryRegionData()
    );
    dlog('bravo region query prepared');
    ...
    my %rec;
    $sth->bind_columns( map { \$rec{$_} } @{ $connection->sql->{regionDataFields} } );
    dlog('Columns binded');

    ilog('Executing bravo region query');
    $sth->execute();
    while ( $sth->fetchrow_arrayref ) {
        ...
        ###Create and populate a new Region object
        ...
        dlog( $region->toString );

        if ( defined $self->list->{$key} ) {
            dlog('region exists in cndb');

            $self->list->{$key}->id( $rec{id} );

            if ( !$region->equals( $self->list->{$key} ) ) {
                dlog('region is not equal');
                dlog( $self->list->{$key}->toString );

                $self->list->{$key}->action('UPDATE');
            }
            else {
                dlog('region is equal, setting to complete');
                $self->list->{$key}->action('COMPLETE');
            }
        }
        else {
            elog('region does not exist in cndb');

            #			die('region deleted from CNDB');
        }
    }
    $sth->finish();
