...
sub prepareSourceData {
    ...
    $self->list( CNDB::Delegate::CNDBDelegate->getContactData($connection) );
...
sub doDelta {
    ...
    $connection->prepareSqlQueryAndFields( BRAVO::Delegate::BRAVODelegate->queryContactData() );
    ...
    my %rec;
    $sth->bind_columns( map { \$rec{$_} } @{ $connection->sql->{contactDataFields} } );
    dlog('Columns binded');
    ...
    while ( $sth->fetchrow_arrayref ) {
        ...
        if ( defined $self->list->{$key} ) {
            ...
            elog( 'contact does not exist in cndb ' . $contact->toString );
            $self->list->{$key} = $contact;
