    ...
    eval {
        ...
        while ( $sth->fetchrow_arrayref ) {
            ...
            if ( $prevMapKey ne $mapKey ) {
                     ...
                     || $stagingSoftwareLpar->action eq 'COMPLETE' )
                {
                    ...
                    if ( defined $rec{mapId} ) {
                            ...
                            if ( defined $rec{scanRecordId} ) {
                                     ...
                                     || $rec{scanRecordAction} eq 'COMPLETE' )
                                {
                                    ...
                                    if ( defined $rec{installedSoftwareTypeTableId} ) {
                                        ###SIT=U|C
                                        if (    $rec{installedSoftwareTypeAction} eq 'UPDATE'
                                             || $rec{installedSoftwareTypeAction} eq 'COMPLETE' )
                                        {
                                            dlog("SSL=U,SSLM,SSLM=C,SR,SR=U|C,SIT,SIT=U|C");
                                            $saveSoftwareLpar = 1;
                                        }
                                        ###SIT=D
                                        elsif ( $rec{installedSoftwareTypeAction} eq 'DELETE' ) {
                                            ###BSL
                                            if ( defined $bravoSoftwareLpar->id ) {
                                                ###BSL=A
                                                if ( $bravoSoftwareLpar->status eq 'ACTIVE' ) {
                                                    dlog("SSL=U,SSLM,SSLM=C,SR,SR=U|C,SIT,SIT=D,BSL,BSL=A");
                                                    $saveSoftwareLpar = 1;
                                                }
                                                ###BSL=I
                                                elsif ( $bravoSoftwareLpar->status eq 'INACTIVE' ) {
                                                    dlog("SSL=U,SSLM,SSLM=C,SR,SR=U|C,SIT,SIT=D,BSL,BSL=I");
                                                    $saveSoftwareLpar = 0;
                                                }
                                                else {
                                            ...
                                            ###!BSL
                                                ...
                                                dlog("SSL=U,SSLM,SSLM=C,SR,SR=U|C,SIT,SIT=D,!BSL");
                                                $saveSoftwareLpar = 0;
                                            }
                                        }
                                        else {
                                            die "Invalid action: " . $rec{installedSoftwareTypeAction} . "\n";
                                        }
                                    }
                                    ###!SIT
                                    else {
                                        ...
                                        if ( defined $bravoSoftwareLpar->id ) {
                                            ###BSL=A
                                            if ( $bravoSoftwareLpar->status eq 'ACTIVE' ) {
                                                dlog("SSL=U,SSLM,SSLM=C,SR,SR=U|C,!SIT,BSL,BSL=A");
                                                $saveSoftwareLpar = 1;
                                            }
                                            ###BSL=I
                                            elsif ( $bravoSoftwareLpar->status eq 'INACTIVE' ) {
                                                dlog("SSL=U,SSLM,SSLM=C,SR,SR=U|C,!SIT,BSL,BSL=I");
                                                $saveSoftwareLpar = 0;
                                            }
                                            else {
                                                die "Invalid status: " . $bravoSoftwareLpar->toString() . "\n";
                                        ...
                                        ###!BSL
                                            ...
                                            dlog("SSL=U,SSLM,SSLM=C,SR,SR=U|C,!SIT,!BSL");
                                            $saveSoftwareLpar = 0;
                                        }
                                    }
                                }
                                ###SR=D
                                    ...
                                    if ( defined $bravoSoftwareLpar->id ) {
                                        ###BSL=A
                                        if ( $bravoSoftwareLpar->status eq 'ACTIVE' ) {
                                            dlog("SSL=U,SSLM,SSLM=C,SR,SR=D,BSL,BSL=A");
                                            $saveSoftwareLpar = 1;
                                        }
                                        ###BSL=I
                                        elsif ( $bravoSoftwareLpar->status eq 'INACTIVE' ) {
                                            dlog("SSL=U,SSLM,SSLM=C,SR,SR=D,BSL,BSL=I");
                                            $saveSoftwareLpar = 0;
                                        }
                                        else {
                                            die "Invalid status: " . $bravoSoftwareLpar->toString() . "\n";
                        ...
                        ###SSLM=D
                            ...
                            if ( defined $bravoSoftwareLpar->id ) {
                                ###BSL=A
                                if ( $bravoSoftwareLpar->status eq 'ACTIVE' ) {
                                    dlog("SSL=U,SSLM,SSLM=D,BSL,BSL=A");
                                    $saveSoftwareLpar = 1;
                                }
                                ###BSL=I
                                elsif ( $bravoSoftwareLpar->status eq 'INACTIVE' ) {
                                    dlog("SSL=U,SSLM,SSLM=D,BSL,BSL=I");
                                    $saveSoftwareLpar = 0;
                                }
                                else {
                                    die "Invalid status: " . $bravoSoftwareLpar->toString() . "\n";
                    ...
                    ###!SSLM
                        ...
                        if ( defined $bravoSoftwareLpar->id ) {
                            ###BSL=A
                            if ( $bravoSoftwareLpar->status eq 'ACTIVE' ) {
                                dlog("SSL=U,!SSLM,BSL,BSL=A");
                                $saveSoftwareLpar = 1;
                            }
                            ###BSL=I
                            elsif ( $bravoSoftwareLpar->status eq 'INACTIVE' ) {
                                dlog("SSL=U,!SSLM,BSL,BSL=I");
                                $saveSoftwareLpar = 0;
                            }
                            else {
                                die "Invalid status: " . $bravoSoftwareLpar->toString() . "\n";
                ...
                ###Add software lpar to statistics hash.
                $statistics{'softwareLpars'}++;
            }
            else {
...
"SIT=U|C,BSL,BSL=A,BIS,BIS=A,DT!=M,SIT!=M,!BIT,BIS=SIT,AUTH!=AUTH|1" );
                 ...
                 || $softwareTypeAction eq 'COMPLETE' )
            {
                ...
                if ( $saveInstalledType == 1 ) {
                    ...
                    if ( $self->applyChanges == 1 ) {
                        ...
                        $bravoInstalledType->save($bravoConnection);
                    ...
                    if ( defined $count && $count == 0 ) {
                        ...
                        if ( $self->applyChanges == 1 ) {
                            ...
                            if ( defined $instSwCount && $instSwCount == 0 ) {
                                ...
                                ###Inactivate this software lpar object since no more inst sw.
                                $bravoSoftwareLpar->status('INACTIVE');
                                dlog( "updated bravo software lpar: " . $bravoSoftwareLpar->toString() );

                                dlog("saving bravo software lpar");
                                $bravoSoftwareLpar->save($bravoConnection);
                                dlog("saved bravo software lpar");
            ...
            ###Add installed software to statistics hash.
            $statistics{'installedSoftwares'}++;
        }
        $sth->finish;
        ...
        if ( $self->applyChanges == 1 ) {
            ...
            foreach my $id ( sort keys %stagingMapsToDelete ) {
                my $softwareLparMap = new Staging::OM::SoftwareLparMap();
