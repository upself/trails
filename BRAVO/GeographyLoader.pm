...
sub prepareSourceData {
    ...
    ilog('Acquring geography data');

    $self->list( CNDB::Delegate::CNDBDelegate->getGeographyData($connection) );
...
sub doDelta {
    ...
    dlog('Preparing bravo geography query');
    $connection->prepareSqlQueryAndFields(    
                                           BRAVO::Delegate::BRAVODelegate->queryGeographyData()
    );
    dlog('bravo geography query prepared');
    ...
    my %rec;
    $sth->bind_columns( map { \$rec{$_} } @{ $connection->sql->{geographyDataFields} } );
    dlog('Columns binded');

    ilog('Executing bravo geography query');
    $sth->execute();
    while ( $sth->fetchrow_arrayref ) {

        ###Get the key
        my $key = $rec{id};
        dlog("geography key=$key");
        ...
        dlog( $geography->toString );

        if ( defined $self->list->{$key} ) {
            dlog('geography exists in cndb');

            $self->list->{$key}->id( $rec{id} );

            if ( !$geography->equals( $self->list->{$key} ) ) {
                dlog('geography is not equal');
                dlog( $self->list->{$key}->toString );

                $self->list->{$key}->action('UPDATE');
            }
            else {
                dlog('geography is equal, setting to complete');
                $self->list->{$key}->action('COMPLETE');
            }
        }
        else {
            elog('geography does not exist in cndb');

            #			die('geography deleted from CNDB');
        }
    }
    $sth->finish();
