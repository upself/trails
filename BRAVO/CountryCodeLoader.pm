package BRAVO::CountryCodeLoader;
...
sub load {
    ...
    my $job = "COUNTRY CODE REPLICATION";
    dlog("job=$job");
...
sub prepareSourceData {
    ...
    ilog('Acquring country code data');

    $self->list( CNDB::Delegate::CNDBDelegate->getCountryCodeData($connection) );
...
sub doDelta {
    ...
    dlog('Preparing bravo country code query');
    $connection->prepareSqlQueryAndFields(    
                                           BRAVO::Delegate::BRAVODelegate->queryCountryCodeData()
    );
    dlog('bravo country code query prepared');
    ...
    my %rec;
    $sth->bind_columns( map { \$rec{$_} } @{ $connection->sql->{countryCodeDataFields} } );
    dlog('Columns binded');
    ...
    while ( $sth->fetchrow_arrayref ) {
        ...
        ###Get the key
        my $key = $rec{id};
        dlog("country code key=$key");

        ###Create and populate a new CountryCode object
        my $countryCode = new BRAVO::OM::CountryCode();
        $countryCode->id( $rec{id} );
        ...
        dlog( $countryCode->toString );

        if ( defined $self->list->{$key} ) {
            dlog('country code exists in cndb');

            $self->list->{$key}->id( $rec{id} );

            if ( !$countryCode->equals( $self->list->{$key} ) ) {
                dlog('country code is not equal');
                dlog( $self->list->{$key}->toString );

                $self->list->{$key}->action('UPDATE');
            }
            else {
                dlog('country code is equal, setting to complete');
                $self->list->{$key}->action('COMPLETE');
            }
        }
        else {
            elog('country code does not exist in cndb');

            #			die('country code deleted from CNDB');
        }
    }
    $sth->finish();
