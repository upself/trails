package BRAVO::LicenseTypeLoader;
...
sub load {
    ...
    my $job = "LICENSE TYPE REPLICATION";
    dlog("job=$job");
...
sub prepareSourceData {
    ...
    $self->list( SWCMDelegate->getLicTypeData($connection) );
...
sub doDelta {
    ...
    $connection->prepareSqlQueryAndFields(    
                                           BRAVO::Delegate::BRAVODelegate->queryLicTypeDataAndFields()
    );
    dlog('bravo license type query prepared');
    ...
    my %rec;
    $sth->bind_columns( map { \$rec{$_} } @{ $connection->sql->{licTypeDataFields} } );
    dlog('Columns binded');

    ilog('Executing bravo license type query');
    $sth->execute();
    while ( $sth->fetchrow_arrayref ) {

        ###Get the key
        my $key = $rec{id};
        dlog("license type key=$key");

        ###Create and populate a new license type object
        ...
        dlog( $licType->toString );

        if ( defined $self->list->{$key} ) {
            dlog('license type exists in swcm');

            $self->list->{$key}->id( $rec{id} );

            if ( !$licType->equals( $self->list->{$key} ) ) {
                dlog('license type is not equal');
                dlog( $self->list->{$key}->toString );

                $self->list->{$key}->action('UPDATE');
            }
            else {
                dlog('license type is equal, setting to complete');
                $self->list->{$key}->action('COMPLETE');
            }
        }
        else {
            elog('license type does not exist in swcm');
            die('license type deleted from swcm');
        }
    }
    $sth->finish();
