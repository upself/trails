...
sub load {
 ...
 eval {
  ...
  $sth->bind_columns( map { \$rec{$_} }
     @{ $self->stagingConnection->sql->{softwareLparDataByCustomerIdFields} } );
  dlog("binded columns for software lpar data query");

  ###Excute the query
  ilog("executing software lpar data query");
  $sth->execute(
   $self->customerId, $self->lparId, $self->customerId, $self->lparId,
   $self->customerId, $self->lparId, $self->customerId, $self->lparId,
   $self->customerId, $self->lparId, $self->customerId, $self->lparId
  );
  ...
  while ( $sth->fetchrow_arrayref ) {
   ...
   if ( $prevMapKey ne $mapKey ) {
    ...
    if ( $saveSoftwareLpar == 1 ) {
     ...
     if (
      ...
      && stringEqual( $bravoSoftwareLpar->biosSerial,
       $stagingSoftwareLpar->biosSerial )
      && numericEqualOrBothUndef(
       $bravoSoftwareLpar->processorCount,
       $stagingSoftwareLpar->processorCount
      )
      && stringEqual( $bravoSoftwareLpar->scanTime,
       $stagingSoftwareLpar->scanTime )
      && $bravoSoftwareLpar->status eq $stagingSoftwareLpar->status
       )
     {

      dlog(
       "bravo software lpar identical to staging software lpar, not saving");

      if ($self->loadDeltaOnly == 0
       && $bravoSoftwareLpar->customerId ne '999999' )
      {
       ...
       my $queue =
         Recon::Queue->new( $self->bravoConnection, $bravoSoftwareLpar );
       $queue->add;
      ...
      $bravoSoftwareLpar->biosManufacturer(
       $stagingSoftwareLpar->biosManufacturer );
      ...
      $bravoSoftwareLpar->dedicatedProcessors(
       $stagingSoftwareLpar->dedicatedProcessors );
      $bravoSoftwareLpar->totalProcessors(
       $stagingSoftwareLpar->totalProcessors );
      $bravoSoftwareLpar->sharedProcessors(
       $stagingSoftwareLpar->sharedProcessors );
      $bravoSoftwareLpar->processorType( $stagingSoftwareLpar->processorType );
      $bravoSoftwareLpar->sharedProcByCores(
       $stagingSoftwareLpar->sharedProcByCores );
      $bravoSoftwareLpar->dedicatedProcByCores(
       $stagingSoftwareLpar->dedicatedProcByCores );
      $bravoSoftwareLpar->totalProcByCores(
       $stagingSoftwareLpar->totalProcByCores );
      $bravoSoftwareLpar->alias( $stagingSoftwareLpar->alias );
      $bravoSoftwareLpar->physicalTotalKb(
       $stagingSoftwareLpar->physicalTotalKb );
      $bravoSoftwareLpar->virtualMemory( $stagingSoftwareLpar->virtualMemory );
      $bravoSoftwareLpar->physicalFreeMemory(
       $stagingSoftwareLpar->physicalFreeMemory );
      $bravoSoftwareLpar->virtualFreeMemory(
       $stagingSoftwareLpar->virtualFreeMemory );
      ...
      $bravoSoftwareLpar->biosSerialNumber(
       $stagingSoftwareLpar->biosSerialNumber );
      ...
      $bravoSoftwareLpar->powerOnPassword(
       $stagingSoftwareLpar->powerOnPassword );
      $bravoSoftwareLpar->processorCount(
       $stagingSoftwareLpar->processorCount );
      ...
      if ( $self->applyChanges == 1 ) {
       ...
       if ( $bravoSoftwareLpar->status eq 'INACTIVE' ) {
        ...
        BRAVO::Delegate::BRAVODelegate->inactivateSoftwareLparById(
         $self->bravoConnection, $bravoSoftwareLpar->id, \%statistics );
       }
      }
     }
    }
    ...
    && $rec{mapAction} eq 'DELETE' )
   {
    ...
    dlog( "scan record flagged for delete, added to hash, scanRecordId: "
       . $rec{scanRecordId} );
   }
   ...
   if ( $rec{installedSoftwareType} eq 'TAD4Z' ) {
    ...
    $kbDefId =
      $self->getMainframeFeatureIdByGUID( $self->bravoConnection,
     $scanSwItem->guid );
    if ( !defined $kbDefId ) {
    ...
    my $versionId = undef;
    $versionId =
      $self->getMainframeVersionIdByFeatureId( $self->bravoConnection,
     $kbDefId );
    ...
    $softwareId =
      $self->getProductIdByVersionId( $self->bravoConnection, $versionId );
   ...
   if ( $rec{installedSoftwareType} eq 'SIGNATURE' ) {
    ...
    dlog(
     'bravo installed type to be processed ' . $bravoInstalledType->toString );
   ...
   dlog( "staging software installed type obj="
      . $stagingInstalledType->toString() );
    ...
    if ( defined $bravoInstalledType->id ) {
     ...
     if ( $rec{installedSoftwareType} eq 'TAD4Z' ) {
      ...
      $stagingInstalledType->lastUsed('1970-01-01')
        if ( !defined $stagingInstalledType->lastUsed
       || '' eq $stagingInstalledType->lastUsed );
   ...
   if ( $rec{installedSoftwareType} eq 'SIGNATURE' ) {
    if (
     defined $bravoInstalledType
     && !stringEqualOrBothUndef(
      $stagingInstalledType->path, $bravoInstalledType->path
     )
      )
    {
     $saveInstalledType = 1;
          ...
          if ( defined $bravoInstalledType->id ) {
           ...
           if (
            ...
            && (
             ...
             && ( $bravoInstalledSoftware->discrepancyTypeId !=
...
"SIT=U|C,BSL,BSL=A,BIS,BIS=A,DT!=M,SIT!=M,!BIT,BIS=SIT,AUTH!=AUTH|1"
    ...
    || $softwareTypeAction eq 'COMPLETE' )
   {
    ...
    if ( $saveInstalledSoftware == 1 ) {
     ...
     if ( $rec{installedSoftwareType} eq 'MANUAL' ) {
      ...
      if ( defined $bravoInstalledSoftware->discrepancyTypeId ) {
       ...
       {
        $bravoInstalledSoftware->discrepancyTypeId(
         $self->discrepancyTypeMap->{'VALID'} );
       }
      }
      else {
       $bravoInstalledSoftware->discrepancyTypeId(
        $self->discrepancyTypeMap->{'NONE'} );
      }
     }

     dlog( "new bravo installed software obj="
        . $bravoInstalledSoftware->toString() );
     ...
     if ( $self->applyChanges == 1 ) {
      ...
      if ( $bravoSoftwareLpar->customerId ne '999999' ) {
       ...
       my $queue =
         Recon::Queue->new( $self->bravoConnection, $bravoInstalledSoftware,
        $bravoSoftwareLpar );
       $queue->add;
    ...
    if ( $saveInstalledSoftware == 1 ) {
     ...
     my $count =
       BRAVO::Delegate::BRAVODelegate->getInstalledSoftwareCountById(
      $self->bravoConnection, $bravoInstalledSoftware->id );
     ...
     if ( defined $count && $count == 0 ) {
      ...
      if ( $self->applyChanges == 1 ) {
       ...
       my $instSwCount =
        ...
        $self->bravoConnection, $bravoSoftwareLpar->id );
       dlog("installed software count for software lpar=$instSwCount");

       if (defined $instSwCount
        && $instSwCount == 0
        && $bravoSoftwareLpar->customerId ne '999999' )
       {
        ###Call the recon engine for the object.
   ...
   if ( defined $softwareDiscrepancyHistory ) {
    ...
    ###Create a new object.
    my $bravoSoftwareDiscrepancyHistory =
      new BRAVO::OM::SoftwareDiscrepancyHistory();
    $bravoSoftwareDiscrepancyHistory->installedSoftwareId(
    ...
    if ( $softwareDiscrepancyHistory eq 'OPEN' ) {
     ...
     die "Invalid software discrepancy history flag value: "
       . $softwareDiscrepancyHistory . "\n";
    }
    dlog( "bravo software discrep history object="
       . $bravoSoftwareDiscrepancyHistory->toString() );
   ...
   if ( $deleteManual == 1 ) {
    ...
    dlog( "swasset installed manual software: "
    ...
    my $manualCount =
      SWASSETDelegate->getManualSoftwareCountByComputerSysId(
     $self->swassetConnection, $rec{scanRecordComputerId} );
  ...
  if ( $self->applyChanges == 1 ) {
   ...
   foreach my $id ( sort keys %stagingMapsToDelete ) {
    ...
    dlog(
     "deleted staging software lpar map: " . $softwareLparMap->toString() );
   }
