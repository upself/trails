...
<p>This page displays data exceptions for hardware Box without
	processor.</p>
<br />
<s:if test="%{data.list.size>0}">
	...
	<s:form action="lparHWNPRC" method="post" theme="simple">
		...
		<div class="button-bar">
			<div class="buttons">
				<span class="button-blue"> <input type="button"
					value="Assign all"
					onclick="javascript:showDx('exceptionHwlparAssignAll.htm')" /> <input
					type="button" value="Unassign all"
		...
		<display:table name="data" class="basic-table" id="row"
			summary="Hardware Box  No Processor"
			decorator="org.displaytag.decorator.TotalTableDecorator"
			cellspacing="2" cellpadding="0" requestURI="lparHWNPRC.htm">
			...
			<display:column sortProperty="hardwareLpar.name" title="Name"
				...
				<s:url id="bravoUrl"
					...
					includeContext="false" includeParams="none" />
				<s:a id="hardwareLpar_%{#attr.row.hardwareLpar.id}"
					href="%{bravoUrl}">${row.hardwareLpar.name}</s:a>
			</display:column>
			<display:column property="hardwareLpar.hardware.serial"
				title="Serial" sortable="true" class="date"
			...
			<display:column property="hardwareLpar.hardware.processorCount"
				title="HW Processors" sortable="true" />
