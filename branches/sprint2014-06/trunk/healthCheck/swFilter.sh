...
DIFFBRAVO=/gsa/pokgsa/projects/e/emea_assets/stagingSwFilterNotInBravo.txt
DIFFSTAGING=/gsa/pokgsa/projects/e/emea_assets/bravoSwFilterNotInStaging.txt
...
db2 export to $EXPORTBRAVO of del \
	...
	installed_software, software_lpar, installed_filter where \
	installed_software.id = installed_filter.installed_software_id \
...
db2 export to $EXPORTSTAGING of del \
    modified by nochardel coldel\| \
    "select distinct software_lpar.customer_id, software_lpar.name, \
	scan_record.bank_account_id, software_filter.software_id \
	from software_lpar_map, software_lpar, scan_record, software_filter \
	where \
	software_lpar_map.software_lpar_id = software_lpar.id \
	and software_lpar_map.scan_record_id = scan_record.id \
	and software_filter.scan_record_id = scan_record.id \
	and scan_record.action = 'COMPLETE' \
	and software_lpar_map.action = 'COMPLETE' \
    with ur"
