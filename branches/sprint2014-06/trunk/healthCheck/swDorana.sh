...
DIFFBRAVO=$TMPDIR/stagingSwDoranaNotInBravo.txt
DIFFSTAGING=$TMPDIR/bravoSwDoranaNotInStaging.txt
...
db2 export to $EXPORTBRAVO of del \
    ...
    "select customer.customer_id, name, bank_account_id \
    , software_id from customer, installed_software \
    , software_lpar, installed_dorana_product where \
	installed_software.id = installed_dorana_product.installed_software_id \
...
db2 export to $EXPORTSTAGING of del \
    ...
    ,scan_record.bank_account_id, software_dorana.software_id \
	from software_lpar_map, software_lpar, scan_record, software_dorana \
	where software_lpar_map.software_lpar_id = software_lpar.id \
	and software_lpar_map.scan_record_id = scan_record.id \
	and software_dorana.scan_record_id = scan_record.id \
