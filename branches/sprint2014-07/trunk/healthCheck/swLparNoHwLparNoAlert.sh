...
DIFFBRAVO=$TMPDIR/swLparNoHwLparNoAlert.txt
...
db2 export to $EXPORTBRAVO of del \
    ...
    "select \
     ...
     where \
        ...
        and not exists( select 1 from  hw_sw_composite hsc  where sl.id = hsc.software_lpar_id) \
        and not exists( select 1 from alert_sw_lpar asl where asl.software_lpar_id = sl.id and asl.open = 1) \
    with ur"
