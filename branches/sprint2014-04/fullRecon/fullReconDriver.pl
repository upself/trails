...
CASE WHEN AUS.Open = 0 THEN 'Blue' WHEN DAYS(CURRENT TIMESTAMP) - DAYS(AUS.Creation_Time) > 90 
THEN 'Red' WHEN DAYS(CURRENT TIMESTAMP) - DAYS(AUS.Creation_Time) > 45 
THEN 'Yellow' ELSE 'Green' END ,aus.creation_time 
...
,h.chips as hwChips 
,case when sle.software_lpar_id is null 
then sl.processor_count else sle.processor_count end as swLparProcCount 
,case when ibmb.id is not null then 
COALESCE( CAST( (select pvui.VALUE_UNITS_PER_CORE from pvu_info pvui where pvui.pvu_id=pvum.pvu_id and 
(case when COALESCE( h.PROCESSOR_COUNT / NULLIF(h.CHIPS,0), 0) = 1 then  'SINGLE-CORE' when COALESCE( h.PROCESSOR_COUNT / NULLIF(h.CHIPS,0), 0) = 2 
then  'DUAL-CORE' when COALESCE( h.PROCESSOR_COUNT / NULLIF(h.CHIPS,0), 0) = 4 
then  'QUAD-CORE' when COALESCE( h.PROCESSOR_COUNT / NULLIF(h.CHIPS,0), 0) > 0 
then 'MULTI-CORE' else '' end ) = pvui.PROCESSOR_TYPE  fetch first 1 row only ) as CHAR(8)),'base data missing') 
else 'Non_IBM Product' end as pvuPerCode,instSi.name as instSwName 
, scp.DESCRIPTION as swOwner,aus.remote_user as alertAssignee 
...
,case when l.id is null then '' when lsm.id is null 
then 'No' else 'Yes' end as catalogMatch 
,l.prod_name as licProdName 
,l.version as licVersion 
,CONCAT(CONCAT(RTRIM(CHAR(L.Cap_Type)), '-'), CT.Description) 
,ul.used_quantity ,case when r.id is null then '' when r.machine_level = 0 
then 'No' else 'Yes' end , REPLACE(RTRIM(CHAR(DATE(L.Expire_Date), USA)), '/', '-') 
,l.po_number ,l.cpu_serial ,case when l.ibm_owned = 0 then 'Customer' 
when l.ibm_owned = 1 then 'IBM' else '' end ,l.ext_src_id ,l.record_time 
from  
eaadmin.software_lpar sl left outer join eaadmin.software_lpar_eff sle on sl.id = sle.software_lpar_id and sle.status = 'ACTIVE' and sle.processor_count != 0 
...
,CASE WHEN AUS.Open = 0 THEN 'Blue' WHEN DAYS(CURRENT TIMESTAMP) - DAYS(AUS.Creation_Time) > 90 THEN 'Red' WHEN DAYS(CURRENT TIMESTAMP) - DAYS(AUS.Creation_Time) > 45 THEN 'Yellow' ELSE 'Green' END 
,aus.creation_time 
, case when aus.open = 1 then DAYS(CURRENT TIMESTAMP) - DAYS(AUS.Creation_Time) else days(aus.record_time) - days(aus.creation_time) end 
,sl.name as swLparName 
,h.serial as hwSerial 
...
when COALESCE( h.PROCESSOR_COUNT / NULLIF(h.CHIPS,0), 0) > 0 then 'MULTI-CORE' else '' end ) = pvui.PROCESSOR_TYPE  
fetch first 1 row only ) as CHAR(8)),'base data missing') else 'Non_IBM Product' end as pvuPerCode
...
,CONCAT(CONCAT(RTRIM(CHAR(L.Cap_Type)), '-')
, CT.Description) 
,ul.used_quantity 
,case when r.id is null then '' when r.machine_level = 0 then 'No' else 'Yes' end 
, REPLACE(RTRIM(CHAR(DATE(L.Expire_Date), USA)), '/', '-') 
,l.po_number ,l.cpu_serial 
,case when l.ibm_owned = 0 then 'Customer' when l.ibm_owned = 1 then 'IBM' else '' end 
