...
eval {
	...
	dlog('Creating new processor loader');
	my $loader = new Staging::ProcessorLoader($opt_b);
	dlog('ip address loader created');

	dlog('Calling the processor load method');
	$loader->load(
		TestMode            => $opt_t,
		LoadDeltaOnly       => $opt_d,
		ApplyChanges        => $opt_a,
	...
	dlog('processor load method complete');
};
if ($@) {
	elog($@);
	die $@;
...
sub usage {
	print
	  "processorToStaging.pl -b <bankaccount> -f <full-load-period-hours>"
