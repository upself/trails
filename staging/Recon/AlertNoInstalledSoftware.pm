package Recon::AlertNoInstalledSoftware;
use strict;
use Base::Utils;
use Carp qw( croak );
...
sub recon {
	my $self = shift;
	ilog('Begin recon method of AlertNoInstalledSoftware');
	...
	ilog('End recon method of AlertNoInstalledSoftware');
}

sub hasSW {
	my $self = shift;
	my $SWcount;
	$self->connection->prepareSqlQuery( $self->queryhasSW() );
	my $sth = $self->connection->sql->{hsSW};
...
sub openAlert {
	my $self = shift;
	ilog('Begin OpenAlert method of AlertNoInstalledSoftware');
	my $alertId = $self->getAlert('NOSW');
	...
	Recon::CauseCode::updateCCtable($alert->id, "NOSW", $self->connection );
	...
	ilog('OpenAlert method of AlertNoInstalledSoftware complete');
}

sub closeAlert {
	my ( $self, $save ) = @_;
	ilog('Begin closeAlert method of AlertNoInstalledSoftware');
	...
	Recon::CauseCode::updateCCtable($alert->id, "NOSW", $self->connection ) if ( $save == 1 );
	...
	ilog('CloseAlert method of AlertNoInstalledSoftware complete');
}
...
sub recordHistory {
	my ( $self, $alert ) = @_;
	ilog('Begin recordHistory method of AlertNoInstalledSoftware');
	...
	ilog('Complete recordHistory method of AlertNoInstalledSoftware');
}
