package Recon::AlertInvalidScanTime;
...
sub recon {
    my $self = shift;
    ilog('Begin recon method of AlertInvalidScanTime');
...
sub isValidScantime {
    ...
    if (! defined $self->softwareLpar->scanTime || $self->softwareLpar->scanTime =~ /^1970/) { 	
        return 0;
    }
...
sub openAlert {
    ...
    my $alertId = $self->getAlert('NULLTIME');
    ...
    Recon::CauseCode::resetCCcode( $alert->id, "NULLTIME", $self->connection);
    ...
    ilog('OpenAlert method of AlertInvalidScanTime complete');
}

sub closeAlert {
    my ( $self, $save ) = @_;
    ilog('Begin closeAlert method of AlertInvalidScanTime');
    ...
    ilog('CloseAlert method of AlertInvalidScanTime complete');
}
...
sub saveSoftwareLparAlert {
    ...
    $sth->execute($softwareLparAlert->id, $softwareLparAlert->softwareLparId);
    $sth->finish;
}
...
sub recordHistory {
    my ( $self, $alert ) = @_;
    ilog('Begin recordHistory method of AlertInvalidScanTime');
    ...
    ilog('Complete recordHistory method of AlertInvalidScanTime');
}
