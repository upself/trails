...
sub validate {
	...
	$self->validateTryAndBuy( $self->license->tryAndBuy,
		$self->license->capType, undef, undef );
	...
	foreach my $rId ( keys %{ $self->licenseAllocationData } ) {
		...
		$rValid *= $self->validateLparNameMatch(
			...
			$self->license->lparName,
			$licenseAllocationView->slName,
			$licenseAllocationView->hlName,
			$licenseAllocationView->rId,
			$licenseAllocationView->rtIsManual
		);
		...
		$rValid *= $self->validateEnvironmentMatch(
			$self->license->environment,
			$licenseAllocationView->hServerType,
			$licenseAllocationView->rtIsManual,
			$licenseAllocationView->rId
		);

		$rValid *= $self->validateLicenseSoftwareMap(
			$self->licSwMap->softwareId,
			$licenseAllocationView->rtIsManual,
			$licenseAllocationView->isSoftwareId,
			$licenseAllocationView->rId,
			$self->license->id,
		...
		$rValid *= $self->validateMaintenanceExpiration(
			...
			$self->license->capType,
			$licenseAllocationView->rtIsManual,
			$licenseAllocationView->expireAge,
			$licenseAllocationView->rId,
			$self->license->id
		);
...
sub validateCapacityType {
	my ( $self, $lrmCapType, $licenseCapType, $reconcileId, $licenseId,
		$isManual )
	  = @_;
...
sub validateProcCount {
	my ( $self, $hProcessorCount, $licenseCapType, $reconcileId, $licenseId ) =
	  @_;
...
sub validateMachineTypeMatch {
	...
	if ( ( $licenseCapType == 14 ) && ( $mtType ne 'WORKSTATION' ) ) {
		if ( $isManual == 0 ) {
			dlog(
"license captype 14 used on non-workstation, adding to list to break"
			);
			$self->addToReconcilesToBreak($reconcileId)
			  if defined $reconcileId;
			$self->addToDeleteQueue($licenseId) if defined $licenseId;
...
sub validateLicenseSoftwareMap {
	...
	my $scarletIs =
	  new Recon::ScarletInstalledSoftware( undef, undef, undef, undef );
...
sub validateScheduleF {
	my ( $self, $ibmOwned, $swComplianceMgmt, $scopeName, $reconcileId,
		$isManual )
	  = @_;
...
sub validateMipsGartnerMsu {
	my (
		$self,        $isManual,    $capType,
		$licType,     $machineType, $isMachineLevel,
		$reconcileId, $hCpuMIPS,    $hCpuGartnerMIPS,
		$hCpuMSU,     $hlPartMIPS,  $hlPartGartnerMIPS,
		$hlPartMSU
	  )
	  = @_;
...
sub validateProcessorChip {
	my ( $self, $isManual, $capType, $machineType, $isMachineLevel,
		$reconcileId )
	  = @_;
