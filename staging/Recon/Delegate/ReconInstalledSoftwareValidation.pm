...
sub validateCustomerOwnedAndManaged {
		...
		&& ( $self->installedSoftwareReconData->scopeName eq 'CUSTOCUSTM' ) )
	{
		...
		dlog(
			"reconcile type id " . $self->installedSoftwareReconData->rTypeId );
		...
		dlog(
			"A different reconcile type than automatic CUSTOCUSTM, so invalid."
		);
		return 0;
	}
		...
		&& ( $self->customer->swComplianceMgmt            eq 'YES' ) )
	{
		...
		dlog( "reconcile map id "
			  . $self->reconcileTypeMap->{
				'Customer owned, managed by 3rd party'} );
		...
		dlog(
			"A different reconcile type than automatic CUSTO3RDM, so invalid.");
		return 0;
	}
...
sub validateIBMOwnedIBMManagedCons {
		...
		&& ( $self->installedSoftwareReconData->scopeName eq 'IBMOIBMMSWCO' ) )
	{
		...
		dlog( "reconcile map id "
			  . $self->reconcileTypeMap->{
				'IBM owned, IBM managed SW consumption based'} );
		...
		dlog(
"A different reconcile type than automatic IBMOIBMMSWCO, so invalid."
		);
		return 0;
	}
...
sub validateCustOwnedIBMManagedCons {
		...
		&& ( $self->customer->swComplianceMgmt            eq 'YES' ) )
	{
		...
		{
			dlog(
				"reconciled as Customer owned, IBM managed SW consumption based"
			);
			return 1;
		}

		dlog(
"A different reconcile type than automatic CUSTOIBMMSWCO, so invalid."
		);
		return 0;
	}
...
sub validateLicenseAllocation {
		...
		$self->reconcileTypeMap->{'Automatic license allocation'} )
	{
		...
		while ( $sth->fetchrow_arrayref ) {
			...
			$validation->validateCapacityType( $rec{lrmCapType}, $rec{capType},
				undef, undef, 0 );
			...
			$validation->validateLicenseSoftwareMap( $rec{sId}, 0,
				...
				undef, undef, $rec{extSrcId}, $rec{guid} );    

			###Validate try and buy
			$validation->validateTryAndBuy( $rec{tryAndBuy}, $rec{capType},
				undef, 0 );
...
# skip check of customer agreement for machinelevel closed reconciles in IBMOIBMM
			if (   ( $self->installedSoftwareReconData->rMachineLevel != 1 )
				or
				( $self->installedSoftwareReconData->scopeName ne "IBMOIBMM" ) )
			{
				...
				if ( $rec{pool} == 1 ) {
					...
					{
						dlog(
"customer id does not match lic, adding to list to break"
						);
						$validation->validationCode(0);
					}
				}
...
"Number of processors and NAMED CPU, license with different serialnumber used"
		...
		###PVU
			...
			if (
				...
				|| ( $self->valueUnitsPerCore ==
					-1 )    # added to make the default value invalid
				|| ( $self->installedSoftwareReconData->processorCount == 0 )
			  )
			{
				dlog(
"PVU needed greater than current used or not found in PVU table"
				);
				return 0;
			}
			if ( $machineLevel == 0 ) {
			...
			if ( $licCapType eq '5' ) {
				if ( $self->installedSoftwareReconData->rMachineLevel == 0 ) {
					...
					elsif ( $self->installedSoftwareReconData->hlPartMIPS !=
						$usedQuantity )
					{
				...
				elsif ( $self->installedSoftwareReconData->hCpuMIPS !=
					$usedQuantity )
				{
				...
				if ( $self->installedSoftwareReconData->rMachineLevel == 0 ) {
					...
					elsif ( $self->installedSoftwareReconData->hlPartMSU !=
						$usedQuantity )
					{
				...
				elsif ( $self->installedSoftwareReconData->hCpuMSU !=
					$usedQuantity )
				{
	...
	my $query = '
        ...
        from
            ...
            join license_sw_map lsm on 
                lsm.license_id = l.id
            join installed_software is on
            	is.id = r.installed_software_id
            join kb_definition kbd on
            	kbd.id = is.software_id
        where
