...
sub reconcile {
 ...
 if ( defined $licsToAllocate ) {
  ...
  foreach my $lId (@usedLicenseId) {
   Recon::Delegate::ScarletDelegate->reconcile( $self->installedSoftware->id,
    $lId );
  }
...
sub attemptExistingMachineLevel {
 ...
 foreach my $reconcileId ( sort keys %{$reconciles} ) {
  ...
  my $recon =
    Recon::InstalledSoftware->new( $self->connection, $installedSoftware );
  $recon->setUp;
