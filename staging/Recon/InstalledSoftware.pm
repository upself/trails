...
#		}
	...
	foreach my $lId ( keys %data ) {
		my $licView = $data{$lId};
		dlog("Validating license $lId of potential pool...");
...
##		$installedSoftwareReconData->scopeName( $rec{scopeName} );
	...
	###Reading new scope from scheduleF, procedure added by Michal Starek
	
	my ( $scopename_temp, $priofound_temp ) = Recon::Delegate::ReconDelegate->getScheduleFScope( $connection,
