package Recon::LicensingInstalledSoftware;
...
sub validateScope {
 ...
 $validation->installedSoftwareReconData( $self->installedSoftwareReconData );
...
sub recon {
 ...
 if ( $validation->validationCode == 0 ) {
  ...
  my ( $returnCode, $scarletInstalledSoftware ) =
    $self->reconcile;    # myyysha - this sometimes returns ( 1, undef )
 ...
 if ( $swLpar->status eq 'ACTIVE' ) {
  my $recon = Recon::SoftwareLpar->new( $self->connection, $swLpar, undef );
...
sub reconcile {
 ...
 if ( defined $licsToAllocate ) {
  ...
  foreach my $lId ( keys %{$licsToAllocate} ) {
   ...
   my $rul =
     $self->createReconcileUsedLicense( $lId, $rId, $licsToAllocate->{$lId},
    $machineLevel, $reconcileIdForMachineLevel );

   ###Append correspond ext src id of the license into scarlet installed software.
   $scarletInstalledSw->appendData( $freePoolData, $lId, $rul->usedLicenseId );
  }
...
sub attemptSoftwareCategory {
 ...
 if ( defined $self->installedSoftwareReconData->scParent ) {
  ...
  ###Create reconcile and set id in data.
  $self->createReconcile( $reconcileTypeMap->{'Covered by software category'},
...
sub attemptExistingMachineLevel {
 ...
 foreach my $reconcileId ( sort keys %{$reconciles} ) {
  ...
  $validation->installedSoftwareReconData( $recon->installedSoftwareReconData );
  $validation->discrepancyTypeMap(
   BRAVO::Delegate::BRAVODelegate->getDiscrepancyTypeMap() );
  $validation->reconcileTypeMap(
   Recon::Delegate::ReconDelegate->getReconcileTypeMap() );
...
sub attemptLicenseAllocation {
 ...
 if ( $scheduleFlevel < 3 ) {    # skip for hostname-specific scheduleF
  ...
  return ( $licsToAllocate, $reconcileTypeMap->{'Automatic license allocation'},
...
sub attemptLicenseAllocationPVU {
 ...
 foreach my $lId ( keys %{$freePoolData} ) {
  ...
  my $neededQuantity         =
    ( $processorCount * $valueUnitsPerProcessor ) - $tempQuantityAllocated;
  dlog( "neededQuantity=" . $neededQuantity );
...
"start calculating PVU, coresPerChip=$nbrCoresPerChip processorCount=$processorCount"
 ...
 $pvuMap->processorBrand( $self->installedSoftwareReconData->hProcessorBrand );
 $pvuMap->processorModel( $self->installedSoftwareReconData->hProcessorModel );
...
sub closeAlertUnlicensedSoftware {
 ...
 if ( grep { $_ eq $self->installedSoftwareReconData->sMfg } $self->ibmArray ) {
  $alert->type('IBM');
 }
 else {
  $alert->type('ISV');
...
sub addChildrenToQueue {
 ...
 if ( defined $self->installedSoftwareReconData->scChildren ) {
  foreach
    my $childId ( keys %{ $self->installedSoftwareReconData->scChildren } )
  {
 ...
 if ( defined $self->installedSoftwareReconData->bChildren ) {
  foreach my $childId ( keys %{ $self->installedSoftwareReconData->bChildren } )
  {
