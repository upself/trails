...
sub recon {
	my $self = shift;
	ilog('Begin recon method of AlertZeroProcessorCount');
	...
	elsif ( $self->isProcessorCountZero == 0 ) {
		$self->closeAlert(0);
	}
	else {
		$self->openAlert;
...
sub isProcessorCountZero {
	...
	if ( defined $effCount && $effCount > 0 ) {
		return 0;
	}
	return 1;
}

sub queryIsProcessorCountZero {
	my $query = '
        select
            sl.processor_count
            ,sle.processor_count
...
sub openAlert {
	my $self = shift;
	ilog('Begin OpenAlert method of AlertInvalidScanTime');
	my $alertId = $self->getAlert('ZEROPROC');
	...
	Recon::CauseCode::updateCCtable ( $alert->id, "ZEROPROC", $self->connection);
	...
	ilog('OpenAlert method of AlertZeroProcessorCount complete');
}

sub closeAlert {
	my ( $self, $save ) = @_;
	ilog('Begin closeAlert method of AlertZeroProcessorCount');
	...
	Recon::CauseCode::updateCCtable ( $alert->id, "ZEROPROC", $self->connection) if ( $save == 1 );
	...
	ilog('CloseAlert method of AlertZeroProcessorCount complete');
}
...
sub recordHistory {
	my ( $self, $alert ) = @_;
	ilog('Begin recordHistory method of AlertZeroProcessorCount');
	...
	ilog('Complete recordHistory method of AlertZeroProcessorCount');
}
