...
sub prepareSourceData {
    ...
    ilog('Acquring CNDB pod data');

    $self->list( CNDB::Delegate::CNDBDelegate->getPodData($connection) );
...
sub doDelta {
    ...
    dlog('Preparing bravo pod query');
    $connection->prepareSqlQueryAndFields(    
                                           BRAVO::Delegate::BRAVODelegate->queryPodData()
    );
    dlog('bravo pod query prepared');
    ...
    my %rec;
    $sth->bind_columns( map { \$rec{$_} } @{ $connection->sql->{podDataFields} } );
    dlog('Columns binded');

    ilog('Executing bravo pod query');
    $sth->execute();
    while ( $sth->fetchrow_arrayref ) {
        ...
        ###Create and populate a new pod object
        ...
        if ( defined $self->list->{$key} ) {
            dlog('pod exists in cndb');

            $self->list->{$key}->id( $rec{id} );

            if ( !$pod->equals( $self->list->{$key} ) ) {
                dlog('pod is not equal');
                dlog( $self->list->{$key}->toString );

                $self->list->{$key}->action('UPDATE');
            }
            else {
                dlog('pod is equal, setting to complete');
                $self->list->{$key}->action('COMPLETE');
            }
        }
        else {
            elog('pod does not exist in cndb');

            #			die('pod deleted from CNDB');
        }
    }
    $sth->finish();
