...
sub prepareSourceData {
    ...
    ilog('Acquring industry data');

    $self->list( CNDB::Delegate::CNDBDelegate->getIndustryData($connection) );
...
sub doDelta {
    ...
    dlog('Preparing bravo industry query');
    $connection->prepareSqlQueryAndFields(    
                                           BRAVO::Delegate::BRAVODelegate->queryIndustryData()
    );
    dlog('bravo industry query prepared');
    ...
    my %rec;
    $sth->bind_columns( map { \$rec{$_} } @{ $connection->sql->{industryDataFields} } );
    dlog('Columns binded');

    ilog('Executing bravo industry query');
    $sth->execute();
    while ( $sth->fetchrow_arrayref ) {

        ###Get the key
        my $key = $rec{id};
        dlog("industry key=$key");

        ###Create and populate a new industry object
        ...
        dlog( $industry->toString );

        if ( defined $self->list->{$key} ) {
            dlog('industry exists in cndb');

            $self->list->{$key}->id( $rec{id} );

            if ( !$industry->equals( $self->list->{$key} ) ) {
                dlog('industry is not equal');
                dlog( $self->list->{$key}->toString );

                $self->list->{$key}->action('UPDATE');
            }
            else {
                dlog('industry is equal, setting to complete');
                $self->list->{$key}->action('COMPLETE');
            }
        }
        else {
            elog('industry does not exist in cndb');

            #			die('industry deleted from CNDB');
        }
    }
    $sth->finish();
