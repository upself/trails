package BRAVO::OM::BundleSoftware;
...
sub save {
    ...
    if( ! defined $self->id ) {
        ...
        my $sth = $connection->sql->{insertBundleSoftware};
        my $id;
        $sth->bind_columns(\$id);
        $sth->execute(
            $self->softwareId
        ...
        my $sth = $connection->sql->{updateBundleSoftware};
        $sth->execute(
            $self->softwareId
            ,$self->id
        );
...
sub queryInsert {
    my $query = '
        ...
        from
            final table (
        insert into bundle_software (
            software_id
        ) values (
            ?
        ))
    ';
    return ('insertBundleSoftware', $query);
}

sub queryUpdate {
    my $query = '
        update bundle_software
        set
            software_id = ?
        where
            id = ?
    ';
    return ('updateBundleSoftware', $query);
}
...
sub delete {
    ...
    if( defined $self->id ) {
        ...
        my $sth = $connection->sql->{deleteBundleSoftware};
        $sth->execute(
            $self->id
        );
        $sth->finish;
...
sub getById {
    ...
    my $sth = $connection->sql->{getByIdKeyBundleSoftware};
    my $softwareId;
    $sth->bind_columns(
        \$softwareId
    );
