...
sub prepareSourceData {
    ...
    ilog('Acquring sector data');

    $self->list( CNDB::Delegate::CNDBDelegate->getSectorData($connection) );
...
sub doDelta {
    ...
    dlog('Preparing bravo sector query');
    $connection->prepareSqlQueryAndFields(    
                                           BRAVO::Delegate::BRAVODelegate->querySectorData()
    );
    dlog('bravo sector query prepared');
    ...
    my %rec;
    $sth->bind_columns( map { \$rec{$_} } @{ $connection->sql->{sectorDataFields} } );
    dlog('Columns binded');

    ilog('Executing bravo sector query');
    $sth->execute();
    while ( $sth->fetchrow_arrayref ) {
        ...
        ###Create and populate a new sector object
        ...
        dlog( $sector->toString );

        if ( defined $self->list->{$key} ) {
            dlog('sector exists in cndb');

            $self->list->{$key}->id( $rec{id} );

            if ( !$sector->equals( $self->list->{$key} ) ) {
                dlog('sector is not equal');
                dlog( $self->list->{$key}->toString );

                $self->list->{$key}->action('UPDATE');
            }
            else {
                dlog('sector is equal, setting to complete');
                $self->list->{$key}->action('COMPLETE');
            }
        }
        else {
            elog('sector does not exist in cndb');

            #			die('sector deleted from CNDB');
        }
    }
    $sth->finish();
