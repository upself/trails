...
sub prepareSourceData {
    ...
    ilog('Acquring major data');

    $self->list( $self->getMajorData($connection) );
...
sub doDelta {
    ...
    dlog('Preparing bravo major query');
    $connection->prepareSqlQueryAndFields( $self->queryMajorData() );
    dlog('bravo major query prepared');

    dlog('Getting statement handle');
    my $sth = $connection->sql->{majorData};
    dlog('Acquired statement handle');

    dlog('Binding columns');
    my %rec;
    $sth->bind_columns( map { \$rec{$_} }
                        @{ $connection->sql->{majorDataFields} } );
    dlog('Columns binded');

    ilog('Executing bravo major query');
    $sth->execute();
    while ( $sth->fetchrow_arrayref ) {
        ...
        ###Create and populate a new major object
        my $major = new BRAVO::OM::Major();
        $major->id( $rec{id} );
        $major->majorId( $rec{id} );
        $major->name( $rec{name} );
        dlog( $major->toString );

        if ( defined $self->list->{$key} ) {
            dlog('major exists in cndb');

            $self->list->{$key}->id( $rec{id} );

            if ( !$major->equals( $self->list->{$key} ) ) {
                dlog('major is not equal');
                dlog( $self->list->{$key}->toString );

                $self->list->{$key}->action('UPDATE');
            }
            else {
                dlog('major is equal, setting to complete');
                $self->list->{$key}->action('COMPLETE');
            }
        }
        else {
            elog('major does not exist in cndb');
            die('major number deleted from CNDB');
        }
    }
    $sth->finish();
...
sub getMajorData {
    ...
    dlog('Start getMajorData method');
    ...
    while ( $sth->fetchrow_arrayref ) {
        ...
        $major->id( $rec{id} ) if $connection->name ne 'CNDB';
