...
use File::Copy;
use Base::Utils;
use SWASSET::TLCMZLoader;
use Base::ConfigManager;
use Tap::NewPerl; 

###Globals
my $logfile          = "/var/staging/logs/tlcmzToSwasset/tlcmzToSwasset_debug.log";
my $pidFile          = "/tmp/tlcmzToSwasset_debug.pid";
my $configFile          = "/opt/staging/v2/config/tlcmzToSwassetConfig_debug.txt";
...
my $scanFilesRootDir = '/var/ftp/mf_scan/perm/83F300F/SYSA';
...
eval {
	...
	while (1) {
		...
		###Execute load(s).
		$loader = new SWASSET::TLCMZLoader();
		$loader->load(
			TestMode             => $testMode,
