...
###Globals
my $logfile = "/var/staging/logs/bravoArchival/inactiveSoftwareArchival.log";
my $pidFile = "/tmp/inactiveSoftwareArchival.pid";
my $configFile = "/opt/staging/v2/config/inactiveSoftwareArchivalConfig.txt";
...
eval {
    ...
    my @softwareIds;
    eval { @softwareIds = getSoftwareIds(); };
    if ($@) {
        die $@;
    }
    ...
    foreach my $id ( sort @customerIds ) {
        ...
        foreach my $softwareId ( sort @softwareIds ) {
            ...
            spawnScript( $id, $softwareId, $childLog );
    ...
    foreach my $softwareId ( sort @softwareIds ) {
        ...
        my $childLog = $logfile . "." . $softwareId;
        spawnSoftwareScript( $softwareId, $childLog );
...
sub spawnScript {
    ...
    if ($pid) {
        ...
        my $cmd
            = "$childScript -b $id -s $softwareId -l $childLog -c $configFile -t $testMode";
...
sub spawnSoftwareScript {
    ...
    if ($pid) {
        ...
        ###I am the child, i *CANNOT* return only exit
        my $cmd
            = "/opt/staging/v2/softwareArchivalChild.pl -s $id -l $childLog -c $configFile -t $testMode";
        dlog("spawning child: $cmd");
        `$cmd >>$childLog 2>&1`;
        dlog("Child complete: $cmd");
        exit 0;
...
sub getCustomerIds {
    ...
    $sth->bind_columns( map { \$rec{$_} }
            @{ $connection->sql->{customerIdsFields} } );
    $sth->execute();
...
sub getSoftwareIds {
    ...
    ###Prepare the necessary sql
    $connection->prepareSqlQueryAndFields( querySoftwareIds() );

    my $sth = $connection->sql->{softwareIds};
    my %rec;
    $sth->bind_columns( map { \$rec{$_} }
            @{ $connection->sql->{softwareIdsFields} } );
    $sth->execute();
    while ( $sth->fetchrow_arrayref ) {
        push @ids, $rec{id};
    }
