...
use File::Copy;
use Base::Utils;
use BRAVO::LicenseLoader;    
use Base::ConfigManager;
use Tap::NewPerl; 

###Globals
my $logfile       = "/var/staging/logs/licenseToBravo/licenseToBravo.log";
my $pidFile       = "/tmp/licenseToBravo.pid";
my $configFile          = "/opt/staging/v2/config/licenseToBravoConfig.txt";
...
eval {
    ...
    while (1) {
        ...
        ###Load license data
        $loader = new BRAVO::LicenseLoader($testMode, $loadDeltaOnly, $applyChanges);
        $loader->load;
