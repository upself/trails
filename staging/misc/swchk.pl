...
eval {
	...
	dlog("getting software lpar list");
	my @softwareLparIds = getSoftwareLparIds($bravoConnection);
	dlog( "softwareLparIds count=" . ( scalar @softwareLparIds ) );
	...
	foreach my $softwareLparId (@softwareLparIds) {
		...
		ilog(   "processing softwareLpar: customerId="
...
sub querySoftwareLparIds {
	...
	my $query = '
        ...
        and c.status = \'ACTIVE\'
        and timestampdiff(16, char(current timestamp - sl.scantime)) < 1
    ';
