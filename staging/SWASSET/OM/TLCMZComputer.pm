package SWASSET::OM::TLCMZComputer;
...
sub save {
    ...
    if( ! defined $self->id ) {
        ...
        my $sth = $connection->sql->{insertTLCMZComputer};
        my $id;
        $sth->bind_columns(\$id);
        $sth->execute(
            $self->computerSysId
        ...
        my $sth = $connection->sql->{updateTLCMZComputer};
        $sth->execute(
            $self->computerScantime
            ,$self->tmeObjectId
            ,$self->tmeObjectLabel
...
sub queryInsert {
    ...
    ';
    return ('insertTLCMZComputer', $query);
}

sub queryUpdate {
    my $query = '
        update tlcmz_computer
        set
            computer_scantime = ?
            ,tme_object_id = ?
            ,tme_object_label = ?
        ...
        where
            computer_sys_id = ?
    ';
    return ('updateTLCMZComputer', $query);
}
...
sub delete {
    ...
    if( defined $self->id ) {
        ...
        my $sth = $connection->sql->{deleteTLCMZComputer};
        $sth->execute(
            $self->computerSysId
        );
        $sth->finish;
...
sub getByBizKey {
    ...
    my $sth = $connection->sql->{getByBizKeyTLCMZComputer};
    my $id;
    my $computerScantime;
    my $tmeObjectId;
    my $tmeObjectLabel;
...
sub getById {
    ...
    my $sth = $connection->sql->{getByIdKeyTLCMZComputer};
    my $computerScantime;
    my $tmeObjectId;
    my $tmeObjectLabel;
    my $sysSerNum;
