...
eval {
	...
	dlog('Creating new ip address loader');
	my $loader = new Staging::IpAddressLoader($opt_b);
	dlog('ip address loader created');

	dlog('Calling the ip address load method');
	$loader->load(
		TestMode            => $opt_t,
		LoadDeltaOnly       => $opt_d,
		ApplyChanges        => $opt_a,
	...
	dlog('ip address load method complete');
};
if ($@) {
	elog($@);
	die $@;
...
sub usage {
	print
	  "ipAddressToStaging.pl -b <bankaccount> -f <full-load-period-hours>"
