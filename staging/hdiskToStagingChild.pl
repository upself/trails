...
eval {
	...
	dlog('Creating new hdisk loader');
	my $loader = new Staging::HdiskLoader($opt_b);
	dlog('hdisk loader created');

	dlog('Calling the hdisk load method');
	$loader->load(
		TestMode            => $opt_t,
		LoadDeltaOnly       => $opt_d,
		ApplyChanges        => $opt_a,
	...
	dlog('hdisk load method complete');
};
if ($@) {
	elog($@);
	die $@;
...
sub usage {
	print
	  "hdiskToStaging.pl -b <bankaccount> -f <full-load-period-hours>"
