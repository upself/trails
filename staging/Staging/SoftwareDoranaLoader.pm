package Staging::SoftwareDoranaLoader;
...
use Scan::Delegate::SoftwareDoranaDelegate;
...
sub new {
	...
	dlog('Building new SoftwareDoranaLoader object');
	...
	dlog( 'SoftwareDoranaLoader instantiated for ' . $bankAccountName );
...
sub load {
	...
	my $job = $self->SUPER::bankAccountName . " SOFTWARE DORANA PULL";
...
sub prepareSourceData {
	...
	eval {
		...
		$self->list(
			SoftwareDoranaDelegate->getSoftwareDoranaData(
...
sub doDelta {
	...
	$connection->prepareSqlQuery(
		Staging::Delegate::StagingDelegate->querySoftwareDoranaData() );
	dlog("Prepared staging software filter query");

	###Define our fields
	my @fields = (qw(id softwareDoranaId softwareId action scanRecordId ));
	...
	dlog('Executing staging software dorana query');
	$sth->execute( $self->SUPER::bankAccount->id );
	while ( $sth->fetchrow_arrayref ) {

		my $key =
		    $rec{scanRecordId} . '|'
		  . $rec{softwareDoranaId} . '|'
		  . $rec{softwareId};
		dlog("key=$key");
		...
		$st->softwareId( $rec{softwareId} );
		$st->softwareDoranaId( $rec{softwareDoranaId} );
		$st->action( $rec{action} );
		dlog( $st->toString );

		if ( $self->list->{$key} ) {
			dlog("Dorana exists in bank account");

			###Set the id
			$self->list->{$key}->id( $rec{id} );

			if ( !$st->equals( $self->list->{$key} ) ) {
				dlog("Dorana is not equal");

				if ( $st->action eq 'COMPLETE' ) {
