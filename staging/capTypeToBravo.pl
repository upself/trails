...
use File::Copy;
use Base::Utils;
use BRAVO::CapTypeLoader;
use Base::ConfigManager;
use Tap::NewPerl; 

###Globals
my $logfile       = "/var/staging/logs/capTypeToBravo/capTypeToBravo.log";
my $pidFile       = "/tmp/capTypeDataToBravo.pid";
my $configFile          = "/opt/staging/v2/config/capTypeToBravoConfig.txt";
...
eval {
	...
	while (1) {
		...
		###Load cap type data
		$loader = new BRAVO::CapTypeLoader();
		$loader->load(
			TestMode      => $testMode,
