...
#test sub
############
#Test Case: Hardware status change from ACTIVE to INACTIVE results in no open HW LPAR without SW LPAR alerts.
#Ensure that your test case compares against a current open HW LPAR without SW LPAR alert.
sub test_ChangeStatus2INACTIVE : Test(2) {
	my $self = shift;
	...
	$stagingHW->hardwareStatus('INACTIVE');
	...
	ok( $stagingHW->hardwareStatus eq 'INACTIVE', "Succeeded to update Staging hardware_status to INACTIVE, id= $stagingHWId");
	die "Failed to update Staging hardware_status to 'INACTIVE'!!\n"
	  unless $stagingHW->hardwareStatus eq 'INACTIVE';
