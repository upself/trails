...
eval {
	...
	dlog('Creating new memMod loader');
	my $loader = new Staging::MemModLoader($opt_b);
	dlog('memMod loader created');

	dlog('Calling the memMod load method');
	$loader->load(
		TestMode            => $opt_t,
		LoadDeltaOnly       => $opt_d,
		ApplyChanges        => $opt_a,
	...
	dlog('memMod load method complete');
};
if ($@) {
	elog($@);
	die $@;
...
sub usage {
	print
	  "memModToStaging.pl -b <bankaccount> -f <full-load-period-hours>"
