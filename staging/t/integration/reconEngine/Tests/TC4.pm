...
sub _01_story36172_isScarletAPIInvalid : Test(12) {
 ...
 integration::reconEngine::TestScarletLicenseAPIInvalid->new( $self, $label )
   ->test;
 ...
 integration::reconEngine::TestAlertOpen->new( $self, $label )->test;
 integration::reconEngine::TestLogReconQuitWithError->new( $self, $label )
   ->test;
...
sub sweep : Test( shutdown => 3 ) {
 ...
 integration::reconEngine::TestScarletLicenseAPIValid->new( $self, $label )
   ->test;
}
