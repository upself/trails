...
use File::Copy;
use Base::Utils;
use SWASSET::NewDataManager;
use Base::ConfigManager;
use Tap::NewPerl; 

###Globals
my $logfile     = "/var/staging/logs/swassetDataManager/swassetDataManager.log";
my $pidFile     = "/tmp/swassetDataManager.pid";
my $configFile          = "/opt/staging/v2/config/swassetDataManagerConfig.txt";
...
eval {
	...
	while (1) {
		...
		###Execute load(s).
		$loader = new SWASSET::NewDataManager();
		$loader->load(
			TestMode      => $testMode,
