package Test::Hardware::TestHardware7;
...
#test sub
############
#Test case: Change Hardware for a given Hardware Lpar within different customer where the new Hardware has status of INACTIVE should close alert
sub test_ChangeLparHWId : Test(2) {
	my $self = shift;
	
	my $stagingHWLparId = 178384;	
	my $oldLparHWId = 164617;
	my $newLparHWId = 17;	
	...
	die "Staging new hw HARDWARE_STATUS != 'INACTIVE'" unless $stagingNewHW->hardwareStatus eq 'INACTIVE';
...
sub teardown01_RollbackStagingDB : Test( teardown ) {
	...
	$stagingHWLpar->save( $self->{stagingConnection} );
	diag("Rollback Staging hardware_id for hardware lpar successfully!");
}
