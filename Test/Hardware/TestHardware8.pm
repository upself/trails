...
# Hardware has hardware lpar in hw_sw_composite
 sub test_ChangeStatus2HWCOUNT : Test(8) {
	my $self = shift;

	my $stagingHWId = 73630;
    ...
    my @hw_lpar_array =(92862,92863,99461);
    foreach(@hw_lpar_array)
    {
    	my $hwLparReconId = $self->getBravoObjReconQueueId( $_, 'hwLpar' );
    	diag("hwLparId = $_");
		ok( defined $hwLparReconId, "Hardware lpar in recon queue, id=$hwLparReconId" );
    }
    
    my @sw_lpar_array =(225874,225876,228819);
    foreach(@sw_lpar_array)
    {
    	my $swLparReconId = $self->getBravoObjReconQueueId( $_, 'swLpar' );
    	diag("swLparId = $_");
		ok( defined $swLparReconId, "Software lpar in recon queue, id=$swLparReconId" );
    }
