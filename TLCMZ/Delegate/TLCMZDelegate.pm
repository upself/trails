...
sub getData {
 ...
 if ( defined $atpAcctNum ) {
  ...
  dlog("no matching atp hardware lpar record found for full serial number");
  ...
  foreach my $my_id ( sort keys %{$hardwareLparMap} ) {
   ...
   my $hardwareSerial4 = substr $hardwareLparMap->{$my_id}->{"hardwareSerial"},
     -4;
   if ( $serial4 eq $hardwareSerial4 ) {
    ...
    if ( $machineType eq "MAINFRAME" ) {
     ...
     dlog( "acct num from atp based on serial(4): " . $atpAcctNum );
     last;
    }
   }
  }
 ...
 unless ( ( lc($inputFile) =~ /xml/ ) || ( lc($inputFile) =~ /asc/ ) ) {
   ...
   if ( $csv->parse($_) ) {
    ...
    if ( defined $row[0] && scalar(@row) >= 7 ) {
     ...
     if ( length( $rec{tlcmzProductId} ) > 8 ) {
      wlog("tlcmz prod id > 8 chars on line $lineNum of file $inputFile");
      $lineError = 1;
     }
     ...
     $tlcmzSoftware->productEblmtElgFlag( $rec{productEblmtElgFlag} );
     $tlcmzSoftware->ibmFeatureCode( $rec{ibmFeatureCode} );
     $tlcmzSoftware->productDelIndicator( $rec{productDelIndicator} );
     ...
     my $installedTLCMZSoftware = new SWASSET::OM::InstalledTLCMZSoftware();
     $installedTLCMZSoftware->computerSysId( $computer->computerSysId );
     $installedTLCMZSoftware->tlcmzProductId( $tlcmzSoftware->tlcmzProductId );
     dlog( "installedTLCMZSoftware=" . $installedTLCMZSoftware->toString() );

     ###Add the inst tlcmz sw record to the list
     my $installedTLCMZSoftwareKey =
       $computer->computerSysId . "." . $tlcmzSoftware->tlcmzProductId;
     if ( exists $installedTLCMZSoftwareList{$installedTLCMZSoftwareKey} ) {
 ...
 # this was not a csv or asc file but an xml file -- deal with it
 if ( lc($inputFile) =~ /xml$/ ) {
  my $lineNum   = 0;
  my $lineError = 0;
  my $twig =
    new XML::Twig(
   twig_roots => { 'SoftwareInventoryAndUse/System/Product' => 1 } );

  $twig->parsefile($inputFile);    # build the twig
  my $root      = $twig->root;     # get the root of the twig
  my @softwares = $root->children;

  foreach my $software (@softwares) {
   my $tlcmzProductId = $software->{'att'}->{'swUniqueKeyRef'};
   if ( length($tlcmzProductId) > 8 ) {
    wlog("tlcmz prod id > 8 chars on line $lineNum of file $inputFile");
    $lineError = 1;
   }
   my $tlcmzSoftware = new SWASSET::OM::TLCMZSoftware();
   $tlcmzSoftware->tlcmzProductId($tlcmzProductId);
   my $tlcmzSoftwareKey = $tlcmzSoftware->tlcmzProductId;
   if ( exists $tlcmzSoftwareList{$tlcmzSoftwareKey} ) {
    wlog("this tlcmz sw was already in the list hash");
   }
 ...
 # this was not a csv or xml file but an asc file -- deal with it
 if ( lc($inputFile) =~ /asc$/ ) {
  open ASC, "<$inputFile" or die "$inputFile: $!";
  my $lineNum   = 0;
  my $lineError = 0;
  ...
  while (<ASC>) {
   if (/^P/) {
    ...
    my $tlcmzProductId = substr $_, 1, 8;

    # take out all trailing spaces
    $tlcmzProductId =~ s/ //g;
    if ( length($tlcmzProductId) > 8 ) {
     wlog("tlcmz prod id > 8 chars on line $lineNum of file $inputFile");
     $lineError = 1;
 ...
 return ( \%computerList, \%tlcmzSoftwareList, \%installedTLCMZSoftwareList );
}
