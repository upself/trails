...
$sth->finish;
my $stmt2 = "select software_lpar.action,software_lpar.name,bios_serial,software_lpar.id, bank_account_id,software_lpar.customer_id from software_lpar, scan_record, software_lpar_map 
...
my $swLparId;
my $swLparIdList = "";
while ( my ($slNum, $swLpar) = each %stageLpar ) {
	my $sth3 = $dbhReporting->prepare($stmt3);
	$sth3->execute($stageLpar{$slNum}{name}, $stageLpar{$slNum}{customerId});
	$sth3->bind_col(1, \$swLparId);
	if ( $sth3->fetch ) {
		$stageLpar{$slNum}{trailsSL} = $swLparId;
		if (length($swLparIdList) < 2 ) {
			$swLparIdList = $swLparId;
		} else {
			$swLparIdList .= ",$swLparId";
		}	
	} 
...
my $stmt4 = "select eaadmin.software_lpar.status, eaadmin.software_lpar.name, bios_serial, scantime, account_number, 
customer_name, match_method, eaadmin.software_lpar.id, 
eaadmin.country_code.code, eaadmin.geography.name
from eaadmin.customer, eaadmin.software_lpar 
left outer join eaadmin.country_code on eaadmin.customer.country_code_id = eaadmin.country_code.id 
left outer join eaadmin.region on eaadmin.country_code.region_id = eaadmin.region.id
left outer join eaadmin.geography on eaadmin.geography.id = eaadmin.region.geography_id 
left outer join eaadmin.hw_sw_composite on eaadmin.software_lpar.id = eaadmin.hw_sw_composite.software_lpar_id 
where eaadmin.software_lpar.id in ($swLparIdList) and 
eaadmin.software_lpar.customer_id = eaadmin.customer.customer_id with ur";

$stmt4 = "select eaadmin.software_lpar.status, eaadmin.software_lpar.name, bios_serial, scantime, account_number,
customer_name, match_method, eaadmin.software_lpar.id,
...
where eaadmin.software_lpar.id in ($swLparIdList)
with ur";

print $stmt4;
my $sth4 = $dbhReporting->prepare($stmt4);
my $status;
my $LparName;
my $biosSerial;
my $scanTime;
...
my $countryCode;
my $geography;
$sth4->bind_col(1, \$status);
$sth4->bind_col(2, \$LparName);
$sth4->bind_col(3, \$biosSerial);
$sth4->bind_col(4, \$scanTime);
$sth4->bind_col(5, \$accountNumber);
$sth4->bind_col(6, \$customerName);
$sth4->bind_col(7, \$matchMethod);
$sth4->bind_col(8, \$slId);
$sth4->bind_col(9, \$countryCode);
$sth4->bind_col(10, \$geography);
...
print REPORT "S-action,S-name,S-serial,S-serial4,S-serial5,S-bankAccountId,S-bankAccountName,T-Status,T-LparName," .
