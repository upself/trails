...
days(current_timestamp) - days(ah.creation_time) as alert_age ,
mt.name as machine_type ,
h.serial as serial ,
'' as hardware_lpar_name ,
'' as software_lpar_name ,
'' as software_name from eaadmin.alert_hardware ah ,
eaadmin.hardware h ,
eaadmin.machine_type mt where ah.open = 1 and days(current timestamp) - days(ah.creation_time) > 90 
...
'' as software_lpar_name ,
'' as software_name 
from eaadmin.alert_hardware_cfgdata ah ,
eaadmin.hardware h ,
eaadmin.machine_type mt 
where ah.open = 1 
and days(current timestamp) - days(ah.creation_time) > 90 
and ah.hardware_id = h.id 
and h.machine_type_id = mt.id
union all
select
...
days(current_timestamp) - days(ahl.creation_time) as alert_age ,
mt.name as machine_type ,
h.serial as serial ,
hl.name as hardware_lpar_name ,
'' as software_lpar_name ,
'' as software_name from eaadmin.alert_hw_lpar ahl ,
eaadmin.hardware_lpar hl ,
eaadmin.hardware h ,
eaadmin.machine_type mt 
where ahl.open = 1 
and days(current timestamp) - days(ahl.creation_time) > 90 
...
days(current_timestamp) - days(asl.creation_time) as alert_age ,
'' as machine_type ,
'' as serial ,
'' as hardware_lpar_name ,
...
where asl.open = 1 
and days(current timestamp) - days(asl.creation_time) > 90 
...
mt.name as machine_type ,h.serial as serial ,hl.name as hardware_lpar_name ,
sl.name as software_lpar_name ,
'' as software_name from eaadmin.customer c ,
eaadmin.alert_expired_scan aes ,
eaadmin.software_lpar sl 
left outer join eaadmin.hw_sw_composite hsc 
on hsc.software_lpar_id = sl.id 
left outer join eaadmin.hardware_lpar hl 
on hsc.hardware_lpar_id = hl.id 
left outer join eaadmin.hardware h 
on hl.hardware_id = h.id 
left outer join eaadmin.machine_type mt 
on h.machine_type_id = mt.id 
where aes.open = 1 
and days(current timestamp) - days(sl.scantime) - c.scan_validity > 90 
and aes.software_lpar_id = sl.id 
and sl.customer_id = c.customer_id 
union all 
SELECT
case when aus.type = 'IBM' then 'UNLICENSED_IBM_SW' else 'UNLICENSED_ISV_SW' 
END || CAST(is.id AS CHAR(16)) as id ,
case when aus.type = 'IBM' then 'UNLICENSED_IBM_SW' else 'UNLICENSED_ISV_SW' 
END as display_name ,sl.customer_id 
as customer_id ,days(current_timestamp) - days(aus.creation_time) 
as alert_age ,mt.name 
as machine_type ,h.serial 
as serial ,hl.name 
as hardware_lpar_name ,sl.name 
as software_lpar_name ,si.name 
as software_name 
...
where aus.open = 1 
and days(current timestamp) - days(aus.creation_time) > 90 
