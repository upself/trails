...
DROP TABLE EAADMIN.MQT_OP_METRICS_SWLM;
...
CREATE SUMMARY TABLE EAADMIN.MQT_OP_METRICS_SWLM
(
   ID,
   CUSTOMER_ID,
   DISPLAY_NAME,
...
(
   ...
   (
      ...
      CASE WHEN SL.CUSTOMER_ID IS NULL THEN 'NON-WORKSTATION' WHEN MT.TYPE != 'WORKSTATION' OR MT.TYPE IS NULL THEN 'NON-WORKSTATION' ELSE MT.TYPE END AS ASSET_TYPE ,
      SUM( CASE WHEN ( AUS.REMOTE_USER IS NULL ) THEN NULL
      		 WHEN ( ( RDT.NEXT_DELIVERY IS NOT NULL ) AND ( RDT.NEXT_DELIVERY > CURRENT TIMESTAMP ) AND ( COALESCE ( SF08.SW_FINANCIAL_RESP, SF6.SW_FINANCIAL_RESP, SF4.SW_FINANCIAL_RESP, SF2.SW_FINANCIAL_RESP, SF0.SW_FINANCIAL_RESP, 'BOOO' ) = 'CUSTO' ) AND ( COALESCE ( SF08.SCOPE_ID, SF6.SCOPE_ID, SF4.SCOPE_ID, SF2.SCOPE_ID, SF0.SCOPE_ID, 0 ) = 2 ) ) THEN 0
			 WHEN ( AUS.REMOTE_USER != 'STAGING' ) THEN 1 ELSE 0 END ) AS ASSIGNED ,
      SUM( CASE WHEN ( ( RDT.NEXT_DELIVERY IS NOT NULL ) AND ( RDT.NEXT_DELIVERY > CURRENT TIMESTAMP ) AND ( COALESCE ( SF08.SW_FINANCIAL_RESP, SF6.SW_FINANCIAL_RESP, SF4.SW_FINANCIAL_RESP, SF2.SW_FINANCIAL_RESP, SF0.SW_FINANCIAL_RESP, 'BOOO' ) = 'CUSTO' ) AND ( COALESCE ( SF08.SCOPE_ID, SF6.SCOPE_ID, SF4.SCOPE_ID, SF2.SCOPE_ID, SF0.SCOPE_ID, 0 ) = 2 ) ) THEN 0 
      ...
      SUM( CASE WHEN ( ( PI.LICENSABLE = 1 ) AND ( COALESCE ( SF08.SCOPE_ID, SF6.SCOPE_ID, SF4.SCOPE_ID, SF2.SCOPE_ID, SF0.SCOPE_ID, 1 ) != 1 ) ) THEN 1 ELSE 0 END ) AS ASSET_TOTAL
      FROM EAADMIN.CUSTOMER C
      JOIN EAADMIN.SOFTWARE_LPAR SL ON SL.CUSTOMER_ID = C.CUSTOMER_ID AND C.SW_LICENSE_MGMT = 'YES' AND C.STATUS = 'ACTIVE' AND SL.STATUS = 'ACTIVE'
      JOIN EAADMIN.HW_SW_COMPOSITE HSC ON SL.ID = HSC.SOFTWARE_LPAR_ID
      JOIN EAADMIN.HARDWARE_LPAR HL ON HL.ID = HSC.HARDWARE_LPAR_ID AND HL.STATUS = 'ACTIVE' AND HL.LPAR_STATUS != 'HWCOUNT'
	  ...
	  LEFT OUTER JOIN EAADMIN.MANUFACTURER MAN ON MAN.ID  = COALESCE ( P.MANUFACTURER_ID, MV.MANUFACTURER_ID, MFV.MANUFACTURER_ID )
	  LEFT OUTER JOIN EAADMIN.SCHEDULE_F SF0 ON ( ( SF0.MANUFACTURER_NAME = MAN.NAME) AND SF0.CUSTOMER_ID = SL.CUSTOMER_ID AND SF0.STATUS_ID = 2 AND SF0.LEVEL = 'MANUFACTURER' )
      LEFT OUTER JOIN EAADMIN.SCHEDULE_F SF2 ON ( ( SF2.SOFTWARE_NAME = SI.NAME OR SF2.SOFTWARE_ID = IS.SOFTWARE_ID ) AND SF2.CUSTOMER_ID = SL.CUSTOMER_ID
					AND SF2.STATUS_ID = 2 AND SF2.LEVEL = 'PRODUCT' )
	  LEFT OUTER JOIN EAADMIN.SCHEDULE_F SF4 ON ( ( SF4.SOFTWARE_NAME = SI.NAME OR SF4.SOFTWARE_ID = IS.SOFTWARE_ID ) AND SF4.CUSTOMER_ID = SL.CUSTOMER_ID
	 				AND SF4.STATUS_ID = 2 AND SF4.LEVEL = 'HWOWNER' AND H.OWNER = SF4.HW_OWNER )
	  LEFT OUTER JOIN EAADMIN.SCHEDULE_F SF6 ON ( ( SF6.SOFTWARE_NAME = SI.NAME OR SF6.SOFTWARE_ID = IS.SOFTWARE_ID ) AND SF6.CUSTOMER_ID = SL.CUSTOMER_ID
	 				AND SF6.STATUS_ID = 2 AND SF6.LEVEL = 'HWBOX' AND H.SERIAL = SF6.SERIAL AND MT.NAME = SF6.MACHINE_TYPE )
      LEFT OUTER JOIN EAADMIN.SCHEDULE_F SF08 ON ( ( SF08.SOFTWARE_NAME = SI.NAME OR SF08.SOFTWARE_ID = IS.SOFTWARE_ID ) AND SF08.CUSTOMER_ID = SL.CUSTOMER_ID
					AND SF08.STATUS_ID = 2 AND SF08.LEVEL = 'HOSTNAME' AND SF08.HOSTNAME = SL.NAME )
      ...
      SUM( CASE WHEN ( ( RDT.NEXT_DELIVERY IS NOT NULL ) AND ( RDT.NEXT_DELIVERY > CURRENT TIMESTAMP ) AND ( COALESCE ( SF08.SW_FINANCIAL_RESP, SF6.SW_FINANCIAL_RESP, SF4.SW_FINANCIAL_RESP, SF2.SW_FINANCIAL_RESP, SF0.SW_FINANCIAL_RESP, 'BOOO' ) = 'CUSTO' ) AND ( COALESCE ( SF08.SCOPE_ID, SF6.SCOPE_ID, SF4.SCOPE_ID, SF2.SCOPE_ID, SF0.SCOPE_ID, 0 ) = 2 ) ) THEN 0
      		 WHEN ( DAYS(CURRENT TIMESTAMP) - DAYS(AUS.CREATION_TIME) BETWEEN 91 AND 120 ) THEN 1 ELSE 0 END ) AS RED91 ,
      SUM( CASE WHEN ( ( PI.LICENSABLE = 1 ) AND ( PIS.ID IS NOT NULL ) AND ( COALESCE ( SF08.SCOPE_ID, SF6.SCOPE_ID, SF4.SCOPE_ID, SF2.SCOPE_ID, SF0.SCOPE_ID, 1 ) != 1 ) ) THEN 1 ELSE 0 END ) AS ASSET_TOTAL
      FROM EAADMIN.CUSTOMER C
      JOIN EAADMIN.SOFTWARE_LPAR SL ON SL.CUSTOMER_ID = C.CUSTOMER_ID AND C.SW_LICENSE_MGMT = 'YES' AND C.STATUS = 'ACTIVE' AND SL.STATUS = 'ACTIVE'
      JOIN EAADMIN.HW_SW_COMPOSITE HSC ON SL.ID = HSC.SOFTWARE_LPAR_ID
      JOIN EAADMIN.HARDWARE_LPAR HL ON HL.ID = HSC.HARDWARE_LPAR_ID AND HL.STATUS = 'ACTIVE' AND HL.LPAR_STATUS != 'HWCOUNT'
      ...
      JOIN ( SELECT C.ID, MNAME.AID, MNAME.BID, C.LEVEL, C.CUSTOMER_ID, C.STATUS_ID FROM EAADMIN.PRIORITY_ISV_SW C
			 JOIN ( SELECT A.ID AS AID, B.ID AS BID FROM EAADMIN.MANUFACTURER A JOIN EAADMIN.MANUFACTURER B ON A.NAME = B.NAME WHERE A.ID <= B.ID ORDER BY A.ID ) AS MNAME
			 ON ( MNAME.AID = C.MANUFACTURER_ID AND C.STATUS_ID = 2 ) ) PIS
         ON PIS.BID = COALESCE ( P.MANUFACTURER_ID, MV.MANUFACTURER_ID, MFV.MANUFACTURER_ID )
      LEFT OUTER JOIN EAADMIN.SOFTWARE_ITEM SI ON SI.ID = IS.SOFTWARE_ID
	  LEFT OUTER JOIN EAADMIN.MANUFACTURER MAN ON MAN.ID  = COALESCE ( P.MANUFACTURER_ID, MV.MANUFACTURER_ID, MFV.MANUFACTURER_ID )
	  LEFT OUTER JOIN EAADMIN.SCHEDULE_F SF0 ON ( ( SF0.MANUFACTURER_NAME = MAN.NAME) AND SF0.CUSTOMER_ID = SL.CUSTOMER_ID AND SF0.STATUS_ID = 2 AND SF0.LEVEL = 'MANUFACTURER' )
      LEFT OUTER JOIN EAADMIN.SCHEDULE_F SF2 ON ( ( SF2.SOFTWARE_NAME = SI.NAME OR SF2.SOFTWARE_ID = IS.SOFTWARE_ID ) AND SF2.CUSTOMER_ID = SL.CUSTOMER_ID
      ...
      LEFT OUTER JOIN EAADMIN.MAINFRAME_VERSION MFV ON MFV.ID = MF.VERSION_ID
      LEFT OUTER JOIN EAADMIN.IBM_BRAND IB ON IB.MANUFACTURER_ID = COALESCE ( P.MANUFACTURER_ID, MV.MANUFACTURER_ID, MFV.MANUFACTURER_ID )
      LEFT OUTER JOIN ( SELECT C.ID, MNAME.AID, MNAME.BID, C.LEVEL, C.CUSTOMER_ID, C.STATUS_ID FROM EAADMIN.PRIORITY_ISV_SW C
			 JOIN ( SELECT A.ID AS AID, B.ID AS BID FROM EAADMIN.MANUFACTURER A JOIN EAADMIN.MANUFACTURER B ON A.NAME = B.NAME WHERE A.ID <= B.ID ORDER BY A.ID ) AS MNAME
			 ON ( MNAME.AID = C.MANUFACTURER_ID AND C.STATUS_ID = 2 ) ) PIS
         ON PIS.BID = COALESCE ( P.MANUFACTURER_ID, MV.MANUFACTURER_ID, MFV.MANUFACTURER_ID )
...
ALTER TABLE "EAADMIN "."MQT_OP_METRICS_SWLM" DEACTIVATE ROW ACCESS CONTROL;		
			
SET INTEGRITY FOR "EAADMIN"."MQT_OP_METRICS_SWLM" IMMEDIATE CHECKED;		
		
GRANT CONTROL ON EAADMIN.MQT_OP_METRICS_SWLM TO USER EAADMIN		
;		
GRANT REFERENCES ON EAADMIN.MQT_OP_METRICS_SWLM TO GROUP TRAILPRD		
;		
GRANT SELECT ON EAADMIN.MQT_OP_METRICS_SWLM TO GROUP TRAILPRD		
;		
GRANT REFERENCES ON EAADMIN.MQT_OP_METRICS_SWLM TO GROUP TRAILUPD		
;		
GRANT SELECT ON EAADMIN.MQT_OP_METRICS_SWLM TO GROUP TRAILUPD
;
