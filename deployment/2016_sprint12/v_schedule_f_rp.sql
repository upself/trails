...
where sl.customer_id =  ssf.customer_id 
...
fetch first 1 row only ), null) as sfid	
from 
...
Where isw.status='ACTIVE' and sl.status='ACTIVE' 
;
GRANT CONTROL ON EAADMIN.v_schedule_f TO USER EAADMIN		
;			
GRANT SELECT ON EAADMIN.v_schedule_f TO GROUP TRAILRPT		
;