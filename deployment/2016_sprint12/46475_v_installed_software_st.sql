...
(
   select
   1
   from EAADMIN.installed_filter c
...
(
   select
   1
   from EAADMIN.installed_signature d
   where d.installed_software_id = b.id
)
and not exists
(
   select
   1
   from EAADMIN.installed_sa_product e
   where e.installed_software_id = b.id
)
and not exists
(
   select
   1
   from EAADMIN.installed_tadz f
   where f.installed_software_id = b.id
)
and not exists
(
   select
   1
   from EAADMIN.installed_script g
   where g.installed_software_id = b.id
)
union 
all
select
...
b.discrepancy_type_id ,
c.id ,
c.software_script_id ,
c.bank_account_id ,
d.map_software_version ,
'SCRIPT'
from EAADMIN.software_lpar a ,
EAADMIN.installed_software b ,
EAADMIN.installed_script c ,
EAADMIN.software_script d
where a.id = b.software_lpar_id
and b.id = c.installed_software_id
and c.software_script_id = d.software_script_id
with ur
;
