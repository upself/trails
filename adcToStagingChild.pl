...
eval {
	...
	dlog('Creating new adc loader');
	my $loader = new Staging::AdcLoader($opt_b);
	dlog('adc loader created');

	dlog('Calling the adc load method');
	$loader->load(
		TestMode            => $opt_t,
		LoadDeltaOnly       => $opt_d,
		ApplyChanges        => $opt_a,
...
sub usage {
	print
	  "adcToStaging.pl -b <bankaccount> -f <full-load-period-hours>"
