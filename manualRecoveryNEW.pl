...
use Recon::Recover::RecoveryNEW;
use CNDB::Delegate::CNDBDelegate;

###Globals
my $logfile    = "/var/staging/logs/manualRecovery/manualRecoveryNEW.log";
...
foreach my $accountNumber ( keys %{$data} ) {
    ...
    my $recovery
        = new Recon::Recover::RecoveryNEW( $connection, $cfgMgr->applyChanges,
...
sub usage {
    print "manualRecoveryNEW -a <account> [-h <hostname>] [-f <file> -m 0/1 ]";
    print
        "-m 0 = will not restore manual breaks OR 1 = will restore manual breaks"
        ;    
    exit 0;
}
