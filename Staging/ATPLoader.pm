...
sub getLastUpdate {
	...
	$connection->prepareSqlQueryAndFields(
		Staging::Delegate::StagingDelegate->queryHwLastUpdate() );
	...
	$sth->bind_columns( map { \$rec{$_} }
		  @{ $connection->sql->{hwLastUpdateFields} } );
	dlog('Columns binded');
...
sub doDelta {
	...
	$connection->prepareSqlQueryAndFields(
		Staging::Delegate::StagingDelegate->queryHardwareData(
			$self->SUPER::testMode, 0
		)
	);
	...
	my %rec;
	$sth->bind_columns( map { \$rec{$_} }
		  @{ $connection->sql->{hardwareDataFields} } );
	dlog('Columns binded');
	...
	while ( $sth->fetchrow_arrayref ) {
		...
		###Get the key
		my $key =
		  $rec{machineTypeId} . '|' . $rec{serial} . '|' . $rec{country};
		...
		if ( defined $self->hardware->{$key} ) {
			...
			if ( $self->SUPER::loadDeltaOnly == 1 ) {
				...
				if ( $hardware->action eq 'COMPLETE' ) {
					...
					dlog(
						'Hardware is in update or delete, setting to complete');
					$hardware->action('COMPLETE');
				}
...
sub applyDeltas {
	...
	Staging::Delegate::StagingDelegate->insertCount( $connection,
		$self->count );
...
sub applyHardwareLparDeltas {
	...
	foreach my $key ( keys %{ $self->hardwareLpar } ) {
		...
		$self->hardware->{ $self->hardwareLpar->{$key}->hardwareKey }
		  ->action('UPDATE')
		  if ( !defined
			$self->hardware->{ $self->hardwareLpar->{$key}->hardwareKey }
			->action );

		if ( $self->hardware->{ $self->hardwareLpar->{$key}->hardwareKey }
			->action eq 'COMPLETE' )
		{
			dlog("Skipping this as is complete");
		}
		...
		if ( $self->hardwareLpar->{$key}->action eq 'COMPLETE' ) {
			...
			$self->hardwareLpar->{$key}->hardwareId(
				$self->hardware->{ $self->hardwareLpar->{$key}->hardwareKey }
				  ->id );
			$self->hardwareLpar->{$key}->save($connection);
			$self->addToCount( 'STAGING', 'HARDWARE_LPAR',
				$self->hardwareLpar->{$key}->action );
...
sub applyEffectiveProcessorDeltas {
	...
	foreach my $key ( keys %{ $self->effProcessor } ) {
		...
		if ( $self->effProcessor->{$key}->action eq 'COMPLETE' ) {
			...
			$self->effProcessor->{$key}
			  ->hardwareLparId( $self->hardwareLpar->{$key}->id );
			$self->effProcessor->{$key}->save($connection);
			$self->addToCount( 'STAGING', 'EFFECTIVE_PROCESSOR',
				$self->effProcessor->{$key}->action );
