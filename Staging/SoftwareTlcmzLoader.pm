package Staging::SoftwareTlcmzLoader;
...
use Scan::Delegate::SoftwareTlcmzDelegate;
...
sub new {
	...
	dlog('Building new SoftwareTlcmzLoader object');
	...
	dlog( 'SoftwareTlcmzLoader instantiated for ' . $bankAccountName );
...
sub load {
	...
	my $job = $self->SUPER::bankAccountName . " SOFTWARE TLCMZ PULL";
...
sub prepareSourceData {
	...
	eval {
		...
		$self->list(
			SoftwareTlcmzDelegate->getSoftwareTlcmzData(
...
sub doDelta {
	...
	$connection->prepareSqlQuery(
		Staging::Delegate::StagingDelegate->querySoftwareTlcmzData() );
	dlog("Prepared staging software filter query");

	###Define our fields
	my @fields = (qw(id softwareTlcmzId softwareId action scanRecordId ));
	...
	while ( $sth->fetchrow_arrayref ) {
		...
		$st->softwareTlcmzId( $rec{softwareTlcmzId} );
