...
sub new {
    ...
    $self->accountNumberMap( CNDB::Delegate::CNDBDelegate->getAccountNumberMap );
    dlog("got cust num map");

    ###Get the cust name map
    $self->getCustomerNameMap( CNDB::Delegate::CNDBDelegate->getCustomerNameMap );
...
sub load {
    ...
    eval {
        ...
        $self->prepareSourceData( $stagingConnection, $swcmConnection, $bravoConnection );
        ...
        $self->licenseListKeep( SWCMDelegate->getLicenseListKeep($swcmConnection) );
        ...
        if ( $args{'ApplyChanges'} == 1 ) {
            ...
            ilog( "skipped applying license changes to staging per ApplyChanges arg" );
...
sub prepareSourceData {
    ...
    $self->swcmData(
                     SWCMDelegate->getData(
                                            $swcmConnection, $bravoConnection, $self->accountNumberMap,
                                            $self->customerNameMap, $self->dupCustomerNameMap, $self->testMode,
...
sub getLastUpdate {
    ...
    $stagingConnection->prepareSqlQueryAndFields( Staging::Delegate::StagingDelegate->queryLicenseLastUpdate() );
    ...
    my %rec;
    $sth->bind_columns( map { \$rec{$_} } @{ $stagingConnection->sql->{licenseLastUpdateFields} } );
    dlog('Columns binded');
...
sub doLicenseDelta {
    ...
    $stagingConnection->prepareSqlQueryAndFields(
                                      Staging::Delegate::StagingDelegate->queryLicenseData( $self->testMode, 0 ) );
    ...
    ###Bind our columns
    my %rec;
    $sth->bind_columns( map { \$rec{$_} } @{ $stagingConnection->sql->{licenseDataFields} } );
    ...
    while ( $sth->fetchrow_arrayref ) {
        ...
        $license->environment( $self->getEnvironment( $rec{environment}, undef ) );
        ...
        if ( defined $self->licenseList->{$key} ) {
            ...
            if ( !$license->equals( $self->licenseList->{$key} ) ) {
                ...
                dlog( "swcm license obj=" . $self->licenseList->{$key}->toString() );
...
sub applyLicenseDelta {
    ...
    foreach my $key ( keys %{ $self->licenseList } ) {
        ...
        dlog( "saved lic: [key=$key] " . $self->licenseList->{$key}->toString() );
    }
}
