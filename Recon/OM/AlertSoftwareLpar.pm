package Recon::OM::AlertSoftwareLpar;
...
sub save {
    ...
    if( ! defined $self->id ) {
        ...
        my $sth = $connection->sql->{insertAlertSoftwareLpar};
        my $id;
        $sth->bind_columns(\$id);
        $sth->execute(
            $self->softwareLparId
        ...
        my $sth = $connection->sql->{updateAlertSoftwareLpar};
        $sth->execute(
            $self->softwareLparId
            ,$self->comments
            ,$self->open
...
sub queryInsert {
    ...
    ';
    return ('insertAlertSoftwareLpar', $query);
}

sub queryUpdate {
    my $query = '
        update alert_sw_lpar
        set
            software_lpar_id = ?
            ,comments = ?
            ,open = ?
        ...
        where
            id = ?
    ';
    return ('updateAlertSoftwareLpar', $query);
}
...
sub delete {
    ...
    if( defined $self->id ) {
        ...
        my $sth = $connection->sql->{deleteAlertSoftwareLpar};
        $sth->execute(
            $self->id
        );
        $sth->finish;
...
sub queryDelete {
    my $query = '
        delete from alert_sw_lpar a where
            exists ( select b.id from alert_sw_lpar b where b.id = ?
            and a.software_lpar_id = b.software_lpar_id 
            and a.comments = b.comments 
            and a.open = b.open 
)    ';
    return ('deleteAlertSoftwareLpar', $query);
}

sub getByBizKey {
    my($self, $connection) = @_;
    $connection->prepareSqlQuery($self->queryGetByBizKey());
    my $sth = $connection->sql->{getByBizKeyAlertSoftwareLpar};
    my $id;
    my $comments;
    my $open;
    my $creationTime;
...
sub getById {
    ...
    my $sth = $connection->sql->{getByIdKeyAlertSoftwareLpar};
    my $softwareLparId;
    my $comments;
    my $open;
    my $creationTime;
