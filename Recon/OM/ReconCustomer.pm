package Recon::OM::ReconCustomer;
...
sub save {
    ...
    if( ! defined $self->id ) {
        ...
        my $sth = $connection->sql->{insertReconCustomer};
        my $id;
        $sth->bind_columns(\$id);
        $sth->execute(
            $self->customerId
        ...
        my $sth = $connection->sql->{updateReconCustomer};
        $sth->execute(
            $self->customerId
            ,$self->action
            ,$self->id
...
sub queryInsert {
    ...
    ';
    return ('insertReconCustomer', $query);
}

sub queryUpdate {
    my $query = '
        update recon_customer
        set
            customer_id = ?
            ,action = ?
            ,remote_user = \'STAGING\'
        ...
        where
            id = ?
    ';
    return ('updateReconCustomer', $query);
}
...
sub delete {
    ...
    if( defined $self->id ) {
        ...
        my $sth = $connection->sql->{deleteReconCustomer};
        $sth->execute(
            $self->id
        );
        $sth->finish;
...
sub queryDelete {
    my $query = '
        delete from recon_customer a where
            exists ( select b.id from recon_customer b where b.id = ?
...
sub getByBizKey {
    ...
    my $sth = $connection->sql->{getByBizKeyReconCustomer};
    my $id;
    my $remoteUser;
    my $recordTime;
    $sth->bind_columns(
...
sub queryGetByBizKey {
    my $query = '
        ...
        where
            ...
            and action = ?
    ';
    return ('getByBizKeyReconCustomer', $query);
}

sub getById {
    my($self, $connection) = @_;
    $connection->prepareSqlQuery($self->queryGetById());
    my $sth = $connection->sql->{getByIdKeyReconCustomer};
    my $customerId;
    my $action;
    my $remoteUser;
    my $recordTime;
...
sub queryGetById {
    my $query = '
        ...
        where
            id = ?
    ';
    return ('getByIdKeyReconCustomer', $query);
}
