package Recon::OM::AlertHardware;
...
sub save {
    ...
    if( ! defined $self->id ) {
        ...
        my $sth = $connection->sql->{insertAlertHardware};
        my $id;
        $sth->bind_columns(\$id);
        $sth->execute(
            $self->hardwareId
        ...
        my $sth = $connection->sql->{updateAlertHardware};
        $sth->execute(
            $self->hardwareId
            ,$self->comments
            ,$self->open
...
sub queryInsert {
    ...
    ';
    return ('insertAlertHardware', $query);
}
...
sub delete {
    ...
    if( defined $self->id ) {
        ...
        my $sth = $connection->sql->{deleteAlertHardware};
        $sth->execute(
            $self->id
        );
        $sth->finish;
...
sub getByBizKey {
    ...
    my $sth = $connection->sql->{getByBizKeyAlertHardware};
    my $id;
    my $comments;
    my $open;
    my $creationTime;
...
sub getById {
    ...
    my $sth = $connection->sql->{getByIdKeyAlertHardware};
    my $hardwareId;
    my $comments;
    my $open;
    my $creationTime;
