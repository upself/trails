package Report::Recon::PendingCustomerDecisionByGeoRegionCountryGen;
...
sub new {
    my ($class) = @_;
    my $self = { _pendingCustomerDecisionByGeoRegionCountryList => undef };
...
sub pendingCustomerDecisionByGeoRegionCountryList {
    my ( $self, $value ) = @_;
    $self->{_pendingCustomerDecisionByGeoRegionCountryList} = $value if defined($value);
    return ( $self->{_pendingCustomerDecisionByGeoRegionCountryList} );
}
...
sub genReport {
    ...
    my $job = "Generating pending customer decision by geo, region and country report";
    dlog("job=$job");
...
sub prepareSourceData {
    ...
    $self->pendingCustomerDecisionByGeoRegionCountryList(
                      TRAILS::Delegate::PendingCustomerDecisionDelegate->getByGeoRegionCountryData($trailsConnection) );
...
sub processReportData {
    ...
    ### Loop thru the pendingCustomerDecisionByGeoRegionCountryList array and process the data
    my $size = $self->pendingCustomerDecisionByGeoRegionCountryList;
    ...
    open( DATA, '>/var/http_reports/trails/pendingCustomerDecisionByGeoRegionCountry.tsv' )
      or die "Couldn't open file /var/http_reports/trails/pendingCustomerDecisionByGeoRegionCountry.tsv, $!";
    ...
    $rptLine = "Pending customer decision by geo, region and country" . "\t";
