...
###Check usage.
use vars qw( $opt_d $opt_a $opt_i $opt_l $opt_c $opt_p  $opt_b $opt_e);
getopts("d:a:i:l:c:p:b:e:");
die "Usage: $0"
...
eval {
	...
	ilog(   "starting child: loadDeltaOnly=$opt_d, applyChanges=$opt_a"
		  . ", customerId=$opt_i"
		  . ", logfile=$opt_l, configFile=$opt_c, phase=$opt_p, begin=$opt_b, end=$opt_e" );

	my @lpars =
	  findSoftwareLparsByCustomerIdByDate( $opt_i, $opt_b, $opt_e,
		$stagingConnection );
	foreach my $id (@lpars) {
...
sub findSoftwareLparsByCustomerIdByDate {
	my ( $customerId, $begin, $end, $connection ) = @_;
	...
	$sth->execute( $customerId, $begin, $end );
