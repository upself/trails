...
DIFFBRAVO=$TMPDIR/hwNoLparNoAlert.txt
...
db2 export to $EXPORTBRAVO of del \
    ...
    "select \
     ...
     where \
        ...
        and not exists( select 1 from hardware_lpar hl where hl.hardware_id = h.id) \
        and not exists( select 1 from alert_hardware ah where ah.hardware_id = h.id and ah.open = 1) \
    with ur"
